...
/**
 * Zod schema for {@link ChopsticksPluginManifestV1} — the complete runtime
 ...
 * | `minimumChopsticksVersion`| string?    | legacy alias of `minimumHostVersion`         |
 ...
 *
 * @see PLUGIN_SPEC.md §10.1 — Manifest shape
 * @see {@link ChopsticksPluginManifestV1} — the inferred TypeScript type
 */
export const pluginManifestV1Schema = z.object({
  ...
  minimumChopsticksVersion: z.string().regex(
    /^\d+\.\d+\.\d+(-[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?(\+[0-9A-Za-z-]+(\.[0-9A-Za-z-]+)*)?$/,
    "minimumChopsticksVersion must follow semver (e.g. 1.0.0)",
  ).optional(),
  ...
  if (
    ...
    && manifest.minimumChopsticksVersion
    && manifest.minimumHostVersion !== manifest.minimumChopsticksVersion
  ) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: "minimumHostVersion and minimumChopsticksVersion must match when both are declared",
