import { asNumber, asString, parseJson, parseObject } from "@chopsticks/adapter-utils/server-utils";
...
export function isQwenUnknownSessionError(stdout: string, stderr: string): boolean {
  ...
  return /unknown\s+session|session\s+not\s+found(?:\s+for\s+id)?|session\s+.*\s+not\s+found|resume\s+.*\s+not\s+found|checkpoint\s+.*\s+not\s+found|cannot\s+resume|failed\s+to\s+resume/i.test(
    haystack,
  );
}
...
const QWEN_AUTH_REQUIRED_RE =
  /(?:not\s+authenticated|please\s+authenticate|api[_ ]?key\s+(?:required|missing|invalid)|authentication\s+required|unauthorized|invalid\s+credentials|not\s+logged\s+in|login\s+required|run\s+`?qwen`?\s+and\s+use\s+`?\/auth`?|session\s+not\s+authenticated|oauth\s+required|dashscope_api_key|bailian_coding_plan_api_key)/i;
