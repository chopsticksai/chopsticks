...
This document describes how `@swarmifyx/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into Swarmifyx logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (Swarmifyx `runId`)
