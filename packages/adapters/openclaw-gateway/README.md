...
This document describes how `@chopsticks/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into Chopsticks logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (Chopsticks `runId`)
