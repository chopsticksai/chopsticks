...
import { asString, parseObject } from "@swarmifyx/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Swarmifyx server host.",
        });
      }
    } catch (err) {
      checks.push({
