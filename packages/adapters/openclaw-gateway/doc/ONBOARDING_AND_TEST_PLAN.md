...
## Scope
This plan is now **gateway-only**. Chopsticks supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Chopsticks skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Chopsticks task.
...
```bash
pnpm --filter @chopsticks/server test:run -- openclaw-gateway
pnpm --filter @chopsticks/server typecheck
pnpm --filter @chopsticks/ui typecheck
pnpm --filter chopsticks typecheck
```
