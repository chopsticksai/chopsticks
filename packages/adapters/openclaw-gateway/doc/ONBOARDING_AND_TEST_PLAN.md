...
## Scope
This plan is now **gateway-only**. RunEach supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses RunEach skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a RunEach task.
...
```bash
pnpm --filter @runeachai/server test:run -- openclaw-gateway
pnpm --filter @runeachai/server typecheck
pnpm --filter @runeachai/ui typecheck
pnpm --filter runeach typecheck
```
