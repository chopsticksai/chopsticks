...
## Scope
This plan is now **gateway-only**. Abacus supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Abacus skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Abacus task.
