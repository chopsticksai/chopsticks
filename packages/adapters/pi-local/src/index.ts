...
Use when:
- You want RunEach to run Pi (the AI coding agent) locally as the agent runtime
...
- RunEach requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/runeachs/ and resumed with --session.
