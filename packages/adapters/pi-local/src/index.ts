...
Use when:
- You want Swarmifyx to run Pi (the AI coding agent) locally as the agent runtime
...
- Swarmifyx requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/swarmifyxs/ and resumed with --session.
