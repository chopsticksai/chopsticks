...
import { asString, runChildProcess } from "@runeachai/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.RUNEACH_PI_COMMAND === "string" &&
      process.env.RUNEACH_PI_COMMAND.trim().length > 0
      ? process.env.RUNEACH_PI_COMMAND.trim()
      : "pi";
