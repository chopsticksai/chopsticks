...
Use when:
- You want RunEach to run Cursor Agent CLI locally as the agent runtime
...
- RunEach auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$runeach" and related skills on local runs.
- RunEach auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
