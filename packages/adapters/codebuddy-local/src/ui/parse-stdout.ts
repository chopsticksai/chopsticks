import type { TranscriptEntry } from "@abacus/adapter-utils";
...
export function parseCodeBuddyStdoutLine(line: string, ts: string): TranscriptEntry[] {
  const normalized = normalizeCodeBuddyStreamLine(line);
  ...
  if (type === "system") {
    ...
    if (subtype === "init") {
      const sessionId =
        ...
        asString(parsed.sessionID);
      return [{ kind: "init", ts, model: asString(parsed.model, "codebuddy"), sessionId }];
    }
