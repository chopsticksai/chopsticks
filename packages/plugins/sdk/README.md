# `@chopsticks/plugin-sdk`

Official TypeScript SDK for Chopsticks plugin authors.

- **Worker SDK:** `@chopsticks/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@chopsticks/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@chopsticks/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@chopsticks/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@chopsticks/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@chopsticks/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@chopsticks/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@chopsticks/plugin-sdk/ui/hooks` | Hooks only |
| `@chopsticks/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@chopsticks/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@chopsticks/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@chopsticks/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@chopsticks/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@chopsticks/plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @chopsticks/plugin-sdk
```
...
- Plugin UI bundles run as same-origin JavaScript inside the main Chopsticks app. They can call ordinary Chopsticks HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Chopsticks instance at runtime.
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@chopsticks/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Chopsticks information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@chopsticks/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@chopsticks/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@chopsticks/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@chopsticks/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@chopsticks/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@chopsticks/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@chopsticks/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@chopsticks/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@chopsticks/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@chopsticks/plugin-sdk/ui";
...
```bash
chopsticks-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@chopsticks/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__chopsticks__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__chopsticks__/events` streams `reload` SSE events on UI build changes
