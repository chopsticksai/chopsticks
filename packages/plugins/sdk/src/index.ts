/**
 * `@abacus/plugin-sdk` — Abacus plugin worker-side SDK.
 *
 * This is the main entrypoint for plugin worker code.  For plugin UI bundles,
 * import from `@abacus/plugin-sdk/ui` instead.
 *
 * @example
 * ```ts
 * // Plugin worker entrypoint (dist/worker.ts)
 * import { definePlugin, runWorker, z } from "@abacus/plugin-sdk";
 *
...
// Manifest and constant types re-exported from @abacus/shared
// Plugin authors import manifest types from here so they have a single
// dependency (@abacus/plugin-sdk) for all plugin authoring needs.
export type {
 ...
 * @example
 * ```ts
 * import { z } from "@abacus/plugin-sdk";
 *
