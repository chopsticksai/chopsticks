/**
 * `@chopsticks/plugin-sdk` — Chopsticks plugin worker-side SDK.
 *
 * This is the main entrypoint for plugin worker code.  For plugin UI bundles,
 * import from `@chopsticks/plugin-sdk/ui` instead.
 *
 * @example
 * ```ts
 * // Plugin worker entrypoint (dist/worker.ts)
 * import { definePlugin, runWorker, z } from "@chopsticks/plugin-sdk";
 *
...
// Manifest and constant types re-exported from @chopsticks/shared
// Plugin authors import manifest types from here so they have a single
// dependency (@chopsticks/plugin-sdk) for all plugin authoring needs.
export type {
 ...
 * @example
 * ```ts
 * import { z } from "@chopsticks/plugin-sdk";
 *
