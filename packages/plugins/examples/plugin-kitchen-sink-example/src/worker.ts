...
import type { Goal, Issue } from "@runeachai/shared";
import {
  DEFAULT_CONFIG,
  JOB_KEYS,
  PLUGIN_ID,
...
async function registerToolHandlers(ctx: PluginContext): Promise<void> {
  ...
  ctx.tools.register(
    TOOL_NAMES.companySummary,
    {
      ...
      description: "Summarizes current company counts from the RunEach APIs.",
...
const plugin: RunEachPlugin = definePlugin({
  async setup(ctx) {
    currentContext = ctx;
