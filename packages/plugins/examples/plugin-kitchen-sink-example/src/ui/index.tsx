...
function KitchenSinkIssueCrudDemo({ context }: { context: PluginPageProps["context"] }) {
  ...
  return (
    <Section title="Issue CRUD">
      <div style={mutedTextStyle}>
        This is a regular embedded React page inside Chopsticks calling the board API directly. It creates, updates, and deletes issues for the current company.
      </div>
      {!context.companyId ? (
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "grid", gap: "14px" }}>
        ...
        <Section title="Chopsticks Animation">
          <div style={mutedTextStyle}>
            This is the same Chopsticks ASCII treatment used in onboarding, copied into the example plugin so the package stays self-contained.
          </div>
          <AsciiArtAnimation />
        </Section>
      </div>
...
function KitchenSinkHostIntegrationDemo({ context }: { context: PluginPageProps["context"] }) {
  ...
  return (
    <Section title="Host Integrations">
      <div style={mutedTextStyle}>
        Plugin pages can feel like native Chopsticks pages. This section demonstrates host toasts, company-scoped routing, and reading live heartbeat data from the embedded page.
      </div>
      <div style={subtleCardStyle}>
        <div style={rowStyle}>
...
function KitchenSinkConsole({ context }: { context: { companyId: string | null; companyPrefix?: string | null; projectId?: string | null; entityId?: string | null; entityType?: string | null } }) {
  ...
  return (
    <div style={{ display: "grid", gap: "14px" }}>
      ...
      <Section title="Chopsticks Domain APIs">
    ...
    <form onSubmit={onSubmit} style={{ display: "grid", gap: "18px" }}>
      <div style={{ display: "grid", gap: "12px", gridTemplateColumns: "minmax(0, 1.8fr) minmax(220px, 1fr)" }}>
        <div style={{ display: "grid", gap: "8px" }}>
          <strong>About</strong>
          <div style={{ fontSize: "13px", lineHeight: 1.5 }}>
            Kitchen Sink demonstrates the current Chopsticks plugin API surface in one local, trusted example. It intentionally includes domain mutations, event handling, streams, tools, jobs, webhooks, and local workspace/process demos.
          </div>
