...
Example Chopsticks plugin that demonstrates:
...
```bash
pnpm --filter @chopsticks/plugin-file-browser-example build
pnpm chopsticks plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm chopsticks plugin uninstall chopsticks-file-browser-example --force
```
...
- Optional: use `chopsticks-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
