...
Example Abacus plugin that demonstrates:
...
```bash
pnpm abacus plugin uninstall abacus-file-browser-example --force
```
...
- Optional: use `abacus-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
