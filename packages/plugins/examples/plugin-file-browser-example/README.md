...
Example RunEach plugin that demonstrates:
...
```bash
pnpm --filter @runeachai/plugin-file-browser-example build
pnpm runeach plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm runeach plugin uninstall runeach-file-browser-example --force
```
...
- Optional: use `runeach-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
