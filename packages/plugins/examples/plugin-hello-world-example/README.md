# @runeachai/plugin-hello-world-example
...
- a minimal React widget rendered in the RunEach dashboard
...
```bash
pnpm --filter @runeachai/plugin-hello-world-example build
pnpm runeach plugin install ./packages/plugins/examples/plugin-hello-world-example
```
...
- **Reinstall after pulling.** If you installed a plugin by local path before the server stored `package_path`, the plugin may show status **error** (worker not found). Uninstall and install again so the server persists the path and can activate the plugin:  
  `pnpm runeach plugin uninstall runeach.hello-world-example --force` then  
  `pnpm runeach plugin install ./packages/plugins/examples/plugin-hello-world-example`.
