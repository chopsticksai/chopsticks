...
const STATEMENT_BREAKPOINT = "-- runeach statement breakpoint 69f6f3f1-42fd-46a6-bf17-d1d85f8f3900";
...
function formatSqlLiteral(value: string): string {
  ...
  while (sanitized.includes(tag)) {
    tag = `$runeach_${Math.random().toString(36).slice(2, 8)}$`;
  }
...
export async function runDatabaseBackup(opts: RunDatabaseBackupOptions): Promise<RunDatabaseBackupResult> {
  const filenamePrefix = opts.filenamePrefix ?? "runeach";
