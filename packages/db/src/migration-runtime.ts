...
function buildEmbeddedPostgresConnectionString(port: number, databaseName: string): string {
  return `postgres://${encodeURIComponent("runeach")}:${encodeURIComponent("runeach")}@127.0.0.1:${port}/${databaseName}`;
}
...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://runeach:runeach@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "runeach");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://runeach:runeach@127.0.0.1:${preferredPort}/runeach`,
