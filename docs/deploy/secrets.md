...
RunEach 会使用本地主密钥对落盘的 secrets 进行加密。代理环境变量里的敏感值（API key、token 等）会以加密 secret 引用的形式存储。
...
|----------|-------------|
| `RUNEACH_SECRETS_MASTER_KEY` | 32 字节密钥，支持 base64、hex 或原始字符串 |
| `RUNEACH_SECRETS_MASTER_KEY_FILE` | 自定义 key 文件路径 |
| `RUNEACH_SECRETS_STRICT_MODE` | 设为 `true` 时强制使用 secret ref |
