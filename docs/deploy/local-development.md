---
title: 本地开发
summary: 为本地开发配置 Chopsticks
---

在几乎零外部依赖的前提下本地运行 Chopsticks。
...
不需要 Docker，也不需要外部数据库。Chopsticks 会自动启用内嵌 PostgreSQL。
...
兼容性说明：`~/.chopsticks` 现在是唯一默认的本地 home。旧路径不会再被自动探测；迁移时请把目录移动到 `~/.chopsticks`，或者显式设置 `CHOPSTICKS_HOME`。
