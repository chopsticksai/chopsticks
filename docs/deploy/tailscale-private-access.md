...
summary: 以适合 Tailscale 的主机绑定方式运行 RunEach，并从其他设备访问
---

如果你想通过 Tailscale（或私有 LAN / VPN）访问 RunEach，而不是只在 `localhost` 上使用，就适合看这一页。
...
把 Tailscale IP 或 MagicDNS 主机名与 RunEach 端口组合使用：
...
如果你使用自定义私有主机名访问 RunEach，请把它加入 allowlist：
...
- 私有主机名下出现登录或重定向错误：用 `runeach allowed-hostname` 把它加入允许列表。
