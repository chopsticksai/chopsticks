...
状态：面向用户的指南  
最后更新：2026-02-17  
读者：在 Chopsticks 中配置并运行代理的运营者

## 1. 这个系统做什么

Chopsticks 中的代理不会持续运行。  
...
对于 `claude_local` 和 `codex_local`，Chopsticks 默认假设 CLI 已经在宿主机上安装并完成认证。
...
对于支持恢复的适配器，Chopsticks 会持久化 session ID。
...
- 如果在适配器环境变量或宿主环境中设置了 `ANTHROPIC_API_KEY`，Claude 会优先使用 API key 认证，而不是订阅登录。Chopsticks 会在环境测试中把这一点作为 warning 展示，而不是 hard error。
