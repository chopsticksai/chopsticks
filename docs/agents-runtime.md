...
对于 `claude_local` 和 `codex_local`，Abacus 默认假设 CLI 已经在宿主机上安装并完成认证。
...
对于支持恢复的适配器，Abacus 会持久化 session ID。
...
- 如果在适配器环境变量或宿主环境中设置了 `ANTHROPIC_API_KEY`，Claude 会优先使用 API key 认证，而不是订阅登录。Abacus 会在环境测试中把这一点作为 warning 展示，而不是 hard error。
