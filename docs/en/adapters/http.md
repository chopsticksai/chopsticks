...
The `http` adapter sends a webhook request to an external agent service. The agent runs externally and Chopsticks just triggers it.
...
1. Chopsticks sends a POST request to the configured URL
2. The request body includes the execution context (agent ID, task info, wake reason)
3. The external agent processes the request and calls back to the Chopsticks API
...
The external agent uses `CHOPSTICKS_API_URL` and an API key to call back to Chopsticks.
