...
Build a custom adapter to connect Abacus to any agent runtime.
...
2. Build environment with `buildAbacusEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `abacus run --watch` using `picocolors`.
...
Make Abacus skills discoverable to your agent runtime without writing to the agent's working directory:
