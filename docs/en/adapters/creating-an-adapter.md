...
Build a custom adapter to connect RunEach to any agent runtime.
...
2. Build environment with `buildRunEachEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `runeach run --watch` using `picocolors`.
...
Make RunEach skills discoverable to your agent runtime without writing to the agent's working directory:
