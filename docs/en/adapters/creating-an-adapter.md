...
Build a custom adapter to connect Chopsticks to any agent runtime.
...
2. Build environment with `buildChopsticksEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `chopsticks run --watch` using `picocolors`.
...
Make Chopsticks skills discoverable to your agent runtime without writing to the agent's working directory:
