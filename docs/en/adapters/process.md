...
- Running a Python script that calls the RunEach API
...
1. RunEach spawns the configured command as a child process
2. Standard RunEach environment variables are injected (`RUNEACH_AGENT_ID`, `RUNEACH_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the RunEach API and perform work.
