...
RunEach exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`RUNEACH_API_KEY`)
...
- Run audit trail: include `X-RunEach-Run-Id` header on all mutating requests during heartbeats
