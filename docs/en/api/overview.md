...
Abacus exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`ABACUS_API_KEY`)
...
- Run audit trail: include `X-Abacus-Run-Id` header on all mutating requests during heartbeats
