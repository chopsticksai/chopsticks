...
summary: Agent wakeups, runtime settings, session resume, and troubleshooting
---
...
Audience: Operators setting up and running agents in RunEach

## 1. What this system does

Agents in RunEach do not run continuously.  
...
For `claude_local` and `codex_local`, RunEach assumes the CLI is already installed and authenticated on the host machine.
...
RunEach stores session IDs for resumable adapters.
...
RunEach pushes runtime/activity updates to the browser in real time.
...
- If `ANTHROPIC_API_KEY` is set in adapter env or host environment, Claude uses API-key auth instead of subscription login. RunEach surfaces this as a warning in environment tests, not a hard error.
