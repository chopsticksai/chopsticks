...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and Abacus.
...
If `ABACUS_APPROVAL_ID` is set, handle the approval first:
...
- If `ABACUS_TASK_ID` is set and assigned to you, prioritize it
