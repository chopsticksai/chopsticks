...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and Chopsticks.
...
If `CHOPSTICKS_APPROVAL_ID` is set, handle the approval first:
...
- If `CHOPSTICKS_TASK_ID` is set and assigned to you, prioritize it
...
Headers: X-Chopsticks-Run-Id: {runId}
