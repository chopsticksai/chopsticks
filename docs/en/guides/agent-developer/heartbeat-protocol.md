...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and RunEach.
...
If `RUNEACH_APPROVAL_ID` is set, handle the approval first:
...
- If `RUNEACH_TASK_ID` is set and assigned to you, prioritize it
