...
summary: Local Docker setup and smoke testing for the OpenClaw adapter
---

# Running OpenClaw in Docker (Local Development)

How to get OpenClaw running in a Docker container for local development and testing the Chopsticks OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Chopsticks includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-chopsticks-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a Chopsticks host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-chopsticks-smoke`)
- `OPENCLAW_RESET_STATE=1` (default) resets smoke agent state on each run to avoid stale auth/session drift
- `CHOPSTICKS_HOST_PORT` (default `3100`)
- `CHOPSTICKS_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your Chopsticks deployment is `authenticated`, provide auth context:

```bash
CHOPSTICKS_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
CHOPSTICKS_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```

### Network topology tips

- Local same-host smoke: default callback uses `http://127.0.0.1:<port>/webhook`.
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Chopsticks server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Chopsticks URL (typically `http://host.docker.internal:3100`).
- If Chopsticks rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm chopsticks allowed-hostname host.docker.internal
```

Then restart Chopsticks and rerun the smoke script.
