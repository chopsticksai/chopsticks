...
Abacus is a monorepo with four main layers.
...
│
├── skills/                      # Agent skills
│   └── abacus/               # Core Abacus skill (heartbeat protocol)
│
├── cli/                         # CLI client
...
3. **Agent process** — Adapter spawns the agent (e.g. Claude Code CLI) with Abacus env vars and a prompt
4. **Agent work** — The agent calls Abacus's REST API to check assignments, checkout tasks, do work, and update status
...
Adapters are the bridge between Abacus and agent runtimes. Each adapter is a package with three modules:
...
- **CLI module** — terminal formatter for `abacus run --watch`

Built-in adapters: `claude_local`, `codex_local`, `process`, `http`. You can create custom adapters for any runtime.

## Key Design Decisions

- **Control plane, not execution plane** — Abacus orchestrates agents; it doesn't run them
