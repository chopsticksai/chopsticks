...
Chopsticks is a monorepo with four main layers.
...
│
├── skills/                      # Agent skills
│   └── chopsticks/               # Core Chopsticks skill (heartbeat protocol)
│
├── cli/                         # CLI client
...
3. **Agent process** — Adapter spawns the agent (e.g. Claude Code CLI) with Chopsticks env vars and a prompt
4. **Agent work** — The agent calls Chopsticks's REST API to check assignments, checkout tasks, do work, and update status
...
Adapters are the bridge between Chopsticks and agent runtimes. Each adapter is a package with three modules:
...
- **CLI module** — terminal formatter for `chopsticks run --watch`

Built-in adapters: `claude_local`, `codex_local`, `process`, `http`. You can create custom adapters for any runtime.

## Key Design Decisions

- **Control plane, not execution plane** — Chopsticks orchestrates agents; it doesn't run them
