...
Abacus uses PostgreSQL via Drizzle ORM. There are three ways to run the database.
...
1. Creates `~/.abacus/instances/default/db/` for storage
2. Ensures the `abacus` database exists
3. Runs migrations automatically
4. Starts serving requests

Data persists across restarts. To reset: `rm -rf ~/.abacus/instances/default/db`.
