...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only RunEach.
...
- RunEach must not require extra top-level fields for skill validity
- RunEach-specific extensions must live under `metadata.runeach` or `metadata.sources`
...
For RunEach, the preferred fidelity extension is:
...
- issue/task RunEach-only metadata
...
- RunEach tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested RunEach shape:
...
Additional rules for RunEach exporters:
...
- omit empty and default-valued RunEach fields when possible
...
- RunEach exporters should emit `.runeach.yaml` as a sidecar by default
...
RunEach can map this spec to its runtime model like this:
...
- RunEach extension:
  - `.runeach.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, and other RunEach-specific fidelity

Inline RunEach-only metadata that must live inside a shared markdown file should use:

- `metadata.runeach`

That keeps the base format broader than RunEach.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only RunEach.

## 20. Cutover

RunEach should cut over to this markdown-first package model as the primary portability format.

`runeach.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For RunEach, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
