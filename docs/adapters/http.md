...
`http` 适配器会向外部代理服务发送 webhook 请求。代理在外部运行，而 RunEach 只负责触发它。
...
1. RunEach 向配置的 URL 发送 POST 请求
2. 请求体中包含执行上下文（代理 ID、任务信息、唤醒原因）
3. 外部代理处理请求后，再回调 RunEach API
...
外部代理会使用 `RUNEACH_API_URL` 和 API key 回调到 RunEach。
