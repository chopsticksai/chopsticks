...
- 运行会调用 Chopsticks API 的 Python 脚本
- 执行自定义代理循环
...
1. Chopsticks 将已配置命令作为子进程启动
2. 注入标准的 Chopsticks 环境变量（例如 `CHOPSTICKS_AGENT_ID`、`CHOPSTICKS_API_KEY`）
3. 进程运行直到结束
4. 由退出码决定成功或失败
...
脚本可以使用注入的环境变量向 Chopsticks API 完成认证，并执行实际工作。
