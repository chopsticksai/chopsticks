...
2. 用 `buildAbacusEnv(agent)` 和上下文变量构建环境
...
`format-event.ts` 使用 `picocolors` 为 `abacus run --watch` 美化终端输出。
...
在不写入代理工作目录的前提下，让代理运行时能够发现 Abacus 技能：
