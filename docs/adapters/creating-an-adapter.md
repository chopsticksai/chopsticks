...
构建自定义适配器，把 Chopsticks 接入任意代理运行时。
...
2. 用 `buildChopsticksEnv(agent)` 和上下文变量构建环境
...
`format-event.ts` 使用 `picocolors` 为 `chopsticks run --watch` 美化终端输出。
...
在不写入代理工作目录的前提下，让代理运行时能够发现 Chopsticks 技能：
