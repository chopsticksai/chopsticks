...
该适配器会创建一个临时目录，把 Abacus 技能以符号链接形式放进去，并通过 `--add-dir` 传给 Claude。这样既能让技能可发现，又不会污染代理自己的工作目录。
...
这个命令会把 Abacus 技能安装到 `~/.claude/skills`，创建一个代理 API key，并输出以该代理身份运行所需的 shell 环境变量。
