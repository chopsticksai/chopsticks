---
title: ClipHub 方案
summary: 面向 Chopsticks 团队蓝图、技能与治理模板的 Marketplace 规划
---

# ClipHub：Chopsticks 团队配置 Marketplace

> 一个面向“整家公司 AI 团队配置”的应用商店。它售卖可直接安装到 Chopsticks 公司的团队蓝图、代理蓝图、技能与治理模板。
...
- 兼容性：支持的适配器、要求模型、最低 Chopsticks 版本
- 社会证明：安装量、评分、评论数
- 元数据：版本、发布时间、更新时间、状态
...
首页
  ...
  -> 选择目标 Chopsticks 公司（或新建）
  -> ClipHub 调用 Chopsticks API：
       1. 创建代理
       2. 建立汇报关系
       3. 创建项目与 workspace
       4. 应用治理规则
       5. 部署 skills 与指令文件
  -> 跳转回 Chopsticks，看到新团队已就位
```
...
## 5.3 从 Chopsticks 导出并发布

```txt
在运行中的 Chopsticks 公司中执行“Export as Blueprint”
  -> 导出代理配置（去敏感信息）
...
- 后端：Node.js API（或直接扩展 Chopsticks server）
- 数据库：Postgres
- 支付：Stripe Connect（Marketplace 模式）
- 存储：S3 / R2
- 认证：可复用 Chopsticks auth，或通过 OAuth2 对接

## 9.2 与 Chopsticks 的集成方式

两种选择：

- **Option A**：独立应用，通过 API 调用 Chopsticks 完成安装
- **Option B**：直接作为 Chopsticks UI 的内置模块（如 `/marketplace`）
