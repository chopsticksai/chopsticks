...
COMPANY_ID="${COMPANY_ID:-${CHOPSTICKS_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-chopsticks-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-chopsticks-openclaw-smoke}"
...
Provide one of:
  CHOPSTICKS_AUTH_HEADER=\"Bearer <board-token>\"
  CHOPSTICKS_COOKIE=\"<board-session-cookie>\"
...
log "checking Chopsticks health"
...
assert_status "200"
if ! grep -q "Chopsticks OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.chopsticks.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
