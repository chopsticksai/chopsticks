...
COMPANY_ID="${COMPANY_ID:-${ABACUS_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-abacus-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-abacus-openclaw-smoke}"
...
Provide one of:
  ABACUS_AUTH_HEADER=\"Bearer <board-token>\"
  ABACUS_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Abacus OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.abacus.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
