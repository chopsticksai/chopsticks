...
COMPANY_ID="${COMPANY_ID:-${RUNEACH_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-runeach-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-runeach-openclaw-smoke}"
...
Provide one of:
  RUNEACH_AUTH_HEADER=\"Bearer <board-token>\"
  RUNEACH_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "RunEach OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.runeach.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
