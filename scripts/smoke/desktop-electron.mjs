...
const packagedAppPath = path.resolve(
  options.app ?? path.resolve(desktopPackageDir, "release", "win-unpacked", "RunEach.exe"),
);
...
try {
  ...
  if (mode === "dev") {
    ...
    runPnpm(["--dir", repoRoot, "--filter", "@runeachai/desktop-electron", "build"], {
      cwd: repoRoot,
    });
  ...
  console.log("[desktop-smoke] Waiting for RunEach server...");
  ...
  if (process.env.RUNEACH_DESKTOP_SMOKE_KEEP_OUTPUT !== "true" && process.env.CI !== "true") {
    // Keep output by default in CI; local runs can opt in via RUNEACH_DESKTOP_SMOKE_KEEP_OUTPUT=true.
    // Otherwise leave artifacts in place for inspection.
  }

  if (process.env.RUNEACH_DESKTOP_SMOKE_CLEAN_HOME === "true") {
