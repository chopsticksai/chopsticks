...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Chopsticks data dir to use
  CHOPSTICKS_HOST=127.0.0.1    Host passed to the onboarded server
  CHOPSTICKS_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/chopsticks-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/chopsticks-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export CHOPSTICKS_HOST="${CHOPSTICKS_HOST:-127.0.0.1}"
export CHOPSTICKS_PORT="${CHOPSTICKS_PORT:-3100}"
export CHOPSTICKS_OPEN_ON_LISTEN="${CHOPSTICKS_OPEN_ON_LISTEN:-false}"
...
echo "CHOPSTICKS_HOST: $CHOPSTICKS_HOST"
echo "CHOPSTICKS_PORT: $CHOPSTICKS_PORT"
...
env \
  ...
  HOST="$CHOPSTICKS_HOST" \
  PORT="$CHOPSTICKS_PORT" \
  CHOPSTICKS_OPEN_ON_LISTEN="$CHOPSTICKS_OPEN_ON_LISTEN" \
  pnpm chopsticks onboard --yes --data-dir "$PC_DATA"
