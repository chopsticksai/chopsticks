...
Low-level reference for how Chopsticks packages are built for npm.
...
The CLI package, `chopsticks`, imports code from workspace packages such as:
...
- `npx chopsticks@canary onboard` can install them explicitly
- `npx chopsticks onboard` continues to resolve `latest`
