...
Abacus supports two runtime modes:
...
When running `authenticated` mode, if the only instance admin is `local-board`, Abacus emits a startup warning with a one-time high-entropy claim URL.
