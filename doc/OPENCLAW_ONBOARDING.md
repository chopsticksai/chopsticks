...
1. Start Swarmifyx in auth mode.
```bash
cd <swarmifyx-repo-root>
...
3. In Swarmifyx UI, go to `http://127.0.0.1:3100/CLA/company/settings`.
...
5. Approve the join request in Swarmifyx UI, then confirm the OpenClaw agent appears in CLA agents.
...
- This is a separate approval from Swarmifyx invite approval. You must approve the pending device in OpenClaw itself.
...
- Ask it to create a new CLA issue in Swarmifyx with unique title `OPENCLAW_CASE_C_CREATED_<timestamp>`.
- Verify in Swarmifyx UI that new issue exists.

10. Watch logs during test (optional but helpful):
```bash
docker compose -f /tmp/openclaw-docker/docker-compose.yml -f /tmp/openclaw-docker/.swarmifyx-openclaw.override.yml logs -f openclaw-gateway
```
