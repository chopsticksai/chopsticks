...
1. Start RunEach in auth mode.
```bash
cd <runeach-repo-root>
...
3. In RunEach UI, go to `http://127.0.0.1:3100/CLA/company/settings`.
...
5. Approve the join request in RunEach UI, then confirm the OpenClaw agent appears in CLA agents.
...
- This is a separate approval from RunEach invite approval. You must approve the pending device in OpenClaw itself.
...
- Ask it to create a new CLA issue in RunEach with unique title `OPENCLAW_CASE_C_CREATED_<timestamp>`.
- Verify in RunEach UI that new issue exists.

10. Watch logs during test (optional but helpful):
```bash
docker compose -f /tmp/openclaw-docker/docker-compose.yml -f /tmp/openclaw-docker/.runeach-openclaw.override.yml logs -f openclaw-gateway
```
