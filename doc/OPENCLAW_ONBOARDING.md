...
3. In Abacus UI, go to `http://127.0.0.1:3100/CLA/company/settings`.
...
5. Approve the join request in Abacus UI, then confirm the OpenClaw agent appears in CLA agents.
...
- This is a separate approval from Abacus invite approval. You must approve the pending device in OpenClaw itself.
...
- Ask it to create a new CLA issue in Abacus with unique title `OPENCLAW_CASE_C_CREATED_<timestamp>`.
- Verify in Abacus UI that new issue exists.

10. Watch logs during test (optional but helpful):
```bash
docker compose -f /tmp/openclaw-docker/docker-compose.yml -f /tmp/openclaw-docker/.abacus-openclaw.override.yml logs -f openclaw-gateway
```
