# RunEach Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for RunEach's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main RunEach app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary RunEach HTTP APIs directly.
...
RunEach plugin design is based on the following assumptions:

1. RunEach is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core RunEach business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by RunEach core.
...
2. Let plugins add major capabilities without editing RunEach core.
...
4. Depend on project-local plugin folders such as `.runeach/plugins`.
...
The single RunEach deployment an operator installs and controls.

### 5.2 Company

A first-class RunEach business object inside the instance.
...
A trusted in-process extension loaded directly by RunEach core.
...
An installable instance-wide extension package loaded through the RunEach plugin runtime.
...
RunEach has two extension classes.
...
RunEach already has a concrete workspace model:
...
Plugins live under the RunEach instance directory.

Suggested layout:

- `~/.runeach/instances/default/plugins/package.json`
- `~/.runeach/instances/default/plugins/node_modules/`
- `~/.runeach/instances/default/plugins/.cache/`
- `~/.runeach/instances/default/data/plugins/<plugin-id>/`
...
RunEach should add CLI commands:

- `pnpm runeach plugin list`
- `pnpm runeach plugin install <package[@version]>`
- `pnpm runeach plugin uninstall <plugin-id>`
- `pnpm runeach plugin upgrade <plugin-id> [version]`
- `pnpm runeach plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@runeachai/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface RunEachPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that RunEach agents can use during runs.
...
- RunEach server starts one worker process per installed plugin
...
Receives one typed RunEach domain event.
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@runeachai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@runeachai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@runeachai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@runeachai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@runeachai/plugin-sdk/ui`
...
The `@runeachai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the RunEach secret provider system rather than a plain text input
...
1. Core RunEach data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing RunEach objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual RunEach product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if RunEach later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing RunEach secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm runeach plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the RunEach server. This is a normative requirement, not optional.
...
### 27.1 `@runeachai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@runeachai/plugin-test-harness";
...
For developing a plugin against a running RunEach instance:

- The operator installs the plugin from a local path: `pnpm runeach plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-runeach-plugin`) that scaffolds:

- `package.json` with correct `runeachPlugin` keys
...
- `@runeachai/plugin-workspace-files`
- `@runeachai/plugin-terminal`
- `@runeachai/plugin-git`
- `@runeachai/plugin-linear`
- `@runeachai/plugin-github-issues`
- `@runeachai/plugin-grafana`
- `@runeachai/plugin-runtime-processes`
- `@runeachai/plugin-stripe`
...
- `@runeachai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@runeachai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@runeachai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@runeachai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@runeachai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@runeachai/plugin-sdk/ui`
...
- `@runeachai/plugin-test-harness`
- `create-runeach-plugin` starter template
...
RunEach should not implement a generic in-process hook bag modeled directly after local coding tools.

RunEach should implement:
...
That is the complete target design for the RunEach plugin system.
