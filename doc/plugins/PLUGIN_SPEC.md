# Swarmifyx Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Swarmifyx's plugin and extension architecture.
...
Swarmifyx plugin design is based on the following assumptions:

1. Swarmifyx is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Swarmifyx business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Swarmifyx core.
...
2. Let plugins add major capabilities without editing Swarmifyx core.
...
4. Depend on project-local plugin folders such as `.swarmifyx/plugins`.
...
The single Swarmifyx deployment an operator installs and controls.

### 5.2 Company

A first-class Swarmifyx business object inside the instance.
...
A trusted in-process extension loaded directly by Swarmifyx core.
...
An installable instance-wide extension package loaded through the Swarmifyx plugin runtime.
...
Swarmifyx has two extension classes.
...
Swarmifyx already has a concrete workspace model:
...
Plugins live under the Swarmifyx instance directory.

Suggested layout:

- `~/.swarmifyx/instances/default/plugins/package.json`
- `~/.swarmifyx/instances/default/plugins/node_modules/`
- `~/.swarmifyx/instances/default/plugins/.cache/`
- `~/.swarmifyx/instances/default/data/plugins/<plugin-id>/`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@swarmifyx/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface SwarmifyxPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
  ...
  minimumSwarmifyxVersion?: string;
  capabilities: string[];
  entrypoints: {
    worker: string;
    ui?: string;
...
Plugins may contribute tools that Swarmifyx agents can use during runs.
...
- Swarmifyx server starts one worker process per installed plugin
...
Receives one typed Swarmifyx domain event.
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@swarmifyx/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@swarmifyx/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@swarmifyx/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@swarmifyx/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@swarmifyx/plugin-sdk/ui`
...
The `@swarmifyx/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Swarmifyx secret provider system rather than a plain text input
...
1. Core Swarmifyx data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Swarmifyx objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Swarmifyx product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Swarmifyx later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Swarmifyx secret provider system.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Swarmifyx server. This is a normative requirement, not optional.
...
### 27.1 `@swarmifyx/plugin-test-harness`
...
```ts
import { createTestHarness } from "@swarmifyx/plugin-test-harness";
...
The host should publish a starter template (`create-swarmifyx-plugin`) that scaffolds:

- `package.json` with correct `swarmifyxPlugin` keys
...
- `@swarmifyx/plugin-workspace-files`
- `@swarmifyx/plugin-terminal`
- `@swarmifyx/plugin-git`
- `@swarmifyx/plugin-linear`
- `@swarmifyx/plugin-github-issues`
- `@swarmifyx/plugin-grafana`
- `@swarmifyx/plugin-runtime-processes`
- `@swarmifyx/plugin-stripe`
...
- `@swarmifyx/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@swarmifyx/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@swarmifyx/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@swarmifyx/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@swarmifyx/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@swarmifyx/plugin-sdk/ui`
...
- `@swarmifyx/plugin-test-harness`
- `create-swarmifyx-plugin` starter template
...
Swarmifyx should not implement a generic in-process hook bag modeled directly after local coding tools.

Swarmifyx should implement:
...
That is the complete target design for the Swarmifyx plugin system.
