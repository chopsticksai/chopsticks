...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@abacus/plugin-sdk/ui`) with design tokens and common components.
...
- install `@abacus/plugin-linear` once
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@abacus/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@abacus/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Abacus instance.
...
- **Single SDK package**: `@abacus/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@abacus/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
```ts
import { definePlugin, z } from "@abacus/plugin-sdk";

export default definePlugin({
  id: "@abacus/plugin-linear",
  version: "0.1.0",
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@abacus/plugin-sdk/ui";
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@abacus/plugin-linear`)
...
Package idea: `@abacus/plugin-workspace-files`
...
Package idea: `@abacus/plugin-terminal`
...
Package idea: `@abacus/plugin-git`
...
- optional `events.emit` (e.g. `plugin.@abacus/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@abacus/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@abacus/plugin-linear`
...
Package idea: `@abacus/plugin-github-issues`
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@abacus/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@abacus/plugin-grafana`
...
Package idea: `@abacus/plugin-runtime-processes`
...
Package idea: `@abacus/plugin-stripe`
...
- plugin UI bundle loading, host bridge, `@abacus/plugin-sdk/ui`
...
- `@abacus/plugin-test-harness` and `create-abacus-plugin` starter template
...
8. Ship `@abacus/plugin-test-harness` and `create-abacus-plugin` starter template.
