...
Chopsticks V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Chopsticks-shaped design.

Assumption for this document: Chopsticks is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Chopsticks objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Chopsticks.

The main conclusion is:

- Chopsticks should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Chopsticks should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Chopsticks should use multiple extension classes instead of one generic plugin bag:
...
If Chopsticks does this well, the examples you listed become straightforward:
...
Relevant Chopsticks files reviewed for current extension seams:
...
It is too dangerous for Chopsticks core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Chopsticks — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Chopsticks should absolutely do this.
...
Chopsticks should copy this discipline.
...
Chopsticks should adopt this style for plugin actions, automations, and UI schemas.
...
Chopsticks can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Chopsticks too.

## What Chopsticks Should Not Copy Directly
...
Chopsticks is a control plane for an operator-managed instance with company objects.
...
Chopsticks is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Chopsticks should not auto-load arbitrary code from a workspace repo like `.chopsticks/plugins` or project directories.
...
For Chopsticks, equivalent hooks into:
...
Chopsticks should not allow plugins to silently replace:
...
For Chopsticks it would be risky because it combines:
...
Chopsticks should require an explicit operator install step.

## Why Chopsticks Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Chopsticks |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Chopsticks instance |
...
That means Chopsticks should borrow the good ideas from `opencode` but use a stricter architecture.

## Chopsticks Already Has Useful Pre-Plugin Seams

Chopsticks has several extension-like seams already:
...
This is good news.
Chopsticks does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Chopsticks Plugin Model
...
Chopsticks already has this pattern implicitly:
...
- `plugin` means integration code that talks to Chopsticks through a typed plugin protocol and capability model
...
- invoke explicit Chopsticks actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@chopsticks/plugin-sdk/ui`) with design tokens and common components.
...
Chopsticks should treat plugin installation as a global instance-level action.

Examples:

- install `@chopsticks/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Chopsticks objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Chopsticks already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Chopsticks too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@chopsticks/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@chopsticks/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Chopsticks instance.

A `create-chopsticks-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Chopsticks achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Chopsticks server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Chopsticks's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@chopsticks/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@chopsticks/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For Chopsticks

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@chopsticks/plugin-sdk";

export default definePlugin({
  id: "@chopsticks/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Chopsticks entities
    });
    ...
    ctx.events.on("plugin.@chopsticks/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@chopsticks/plugin-sdk/ui";
...
Any Chopsticks plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Chopsticks objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@chopsticks/plugin-linear`)
...
- reuse core tables when the data is clearly part of Chopsticks itself
...
If a concept is becoming part of Chopsticks's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Chopsticks itself
...
### 3. Scope plugin data to Chopsticks objects before adding custom schemas

A lot of plugin data naturally hangs off existing Chopsticks objects:
...
If Chopsticks eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@chopsticks/plugin-workspace-files`
...
Package idea: `@chopsticks/plugin-terminal`
...
Package idea: `@chopsticks/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Chopsticks.
- Board reviews the diff after a run without leaving Chopsticks.
...
- optional `events.emit` (e.g. `plugin.@chopsticks/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@chopsticks/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@chopsticks/plugin-linear`

This plugin syncs Chopsticks work with Linear. It is useful for:

- importing backlog from Linear
- linking Chopsticks issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Chopsticks and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Chopsticks.
- Chopsticks issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@chopsticks/plugin-github-issues`

This plugin syncs Chopsticks issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Chopsticks issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Chopsticks task governance
...
- Plugin settings:
  ...
  - whether new Chopsticks issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Chopsticks issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Chopsticks.
- GitHub webhooks update status/comment state in Chopsticks.
- A PR is linked back to the Chopsticks issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@chopsticks/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@chopsticks/plugin-grafana`

This plugin surfaces external metrics and dashboards inside Chopsticks. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Chopsticks issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Chopsticks plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Chopsticks dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Chopsticks issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Chopsticks core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@chopsticks/plugin-runtime-processes`
...
Package idea: `@chopsticks/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into Chopsticks. It is useful for:
...
- Failed payment spikes or churn events can generate Chopsticks issues for follow-up.
...
- Stripe data should stay additive to Chopsticks core
...
- plugin UI bundle loading, host bridge, `@chopsticks/plugin-sdk/ui`
...
- `@chopsticks/plugin-test-harness` and `create-chopsticks-plugin` starter template
...
Chopsticks should not implement "an OpenCode-style generic in-process hook system."
Chopsticks should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Chopsticks
...
8. Ship `@chopsticks/plugin-test-harness` and `create-chopsticks-plugin` starter template.
