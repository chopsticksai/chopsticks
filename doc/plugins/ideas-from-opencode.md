...
Swarmifyx V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Swarmifyx-shaped design.

Assumption for this document: Swarmifyx is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Swarmifyx objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Swarmifyx.

The main conclusion is:

- Swarmifyx should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Swarmifyx should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Swarmifyx should use multiple extension classes instead of one generic plugin bag:
...
If Swarmifyx does this well, the examples you listed become straightforward:
...
Relevant Swarmifyx files reviewed for current extension seams:
...
It is too dangerous for Swarmifyx core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Swarmifyx — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Swarmifyx should absolutely do this.
...
Swarmifyx should copy this discipline.
...
Swarmifyx should adopt this style for plugin actions, automations, and UI schemas.
...
Swarmifyx can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Swarmifyx too.

## What Swarmifyx Should Not Copy Directly
...
Swarmifyx is a control plane for an operator-managed instance with company objects.
...
Swarmifyx is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Swarmifyx should not auto-load arbitrary code from a workspace repo like `.swarmifyx/plugins` or project directories.
...
For Swarmifyx, equivalent hooks into:
...
Swarmifyx should not allow plugins to silently replace:
...
For Swarmifyx it would be risky because it combines:
...
Swarmifyx should require an explicit operator install step.

## Why Swarmifyx Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Swarmifyx |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Swarmifyx instance |
...
That means Swarmifyx should borrow the good ideas from `opencode` but use a stricter architecture.

## Swarmifyx Already Has Useful Pre-Plugin Seams

Swarmifyx has several extension-like seams already:
...
This is good news.
Swarmifyx does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Swarmifyx Plugin Model
...
Swarmifyx already has this pattern implicitly:
...
- `plugin` means integration code that talks to Swarmifyx through a typed plugin protocol and capability model
...
- invoke explicit Swarmifyx actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@swarmifyx/plugin-sdk/ui`) with design tokens and common components.
...
Swarmifyx should treat plugin installation as a global instance-level action.

Examples:

- install `@swarmifyx/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Swarmifyx objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Swarmifyx already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Swarmifyx too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@swarmifyx/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@swarmifyx/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Swarmifyx instance.

A `create-swarmifyx-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Swarmifyx achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Swarmifyx server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Swarmifyx's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@swarmifyx/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@swarmifyx/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For Swarmifyx

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@swarmifyx/plugin-sdk";

export default definePlugin({
  id: "@swarmifyx/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Swarmifyx entities
    });
    ...
    ctx.events.on("plugin.@swarmifyx/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@swarmifyx/plugin-sdk/ui";
...
Any Swarmifyx plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Swarmifyx objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@swarmifyx/plugin-linear`)
...
- reuse core tables when the data is clearly part of Swarmifyx itself
...
If a concept is becoming part of Swarmifyx's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Swarmifyx itself
...
### 3. Scope plugin data to Swarmifyx objects before adding custom schemas

A lot of plugin data naturally hangs off existing Swarmifyx objects:
...
If Swarmifyx eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@swarmifyx/plugin-workspace-files`
...
Package idea: `@swarmifyx/plugin-terminal`
...
Package idea: `@swarmifyx/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Swarmifyx.
- Board reviews the diff after a run without leaving Swarmifyx.
...
- optional `events.emit` (e.g. `plugin.@swarmifyx/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@swarmifyx/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@swarmifyx/plugin-linear`

This plugin syncs Swarmifyx work with Linear. It is useful for:

- importing backlog from Linear
- linking Swarmifyx issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Swarmifyx and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Swarmifyx.
- Swarmifyx issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@swarmifyx/plugin-github-issues`

This plugin syncs Swarmifyx issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Swarmifyx issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Swarmifyx task governance
...
- Plugin settings:
  ...
  - whether new Swarmifyx issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Swarmifyx issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Swarmifyx.
- GitHub webhooks update status/comment state in Swarmifyx.
- A PR is linked back to the Swarmifyx issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@swarmifyx/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@swarmifyx/plugin-grafana`

This plugin surfaces external metrics and dashboards inside Swarmifyx. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Swarmifyx issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Swarmifyx plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Swarmifyx dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Swarmifyx issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Swarmifyx core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@swarmifyx/plugin-runtime-processes`
...
Package idea: `@swarmifyx/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into Swarmifyx. It is useful for:
...
- Failed payment spikes or churn events can generate Swarmifyx issues for follow-up.
...
- Stripe data should stay additive to Swarmifyx core
...
- plugin UI bundle loading, host bridge, `@swarmifyx/plugin-sdk/ui`
...
- `@swarmifyx/plugin-test-harness` and `create-swarmifyx-plugin` starter template
...
Swarmifyx should not implement "an OpenCode-style generic in-process hook system."
Swarmifyx should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Swarmifyx
...
8. Ship `@swarmifyx/plugin-test-harness` and `create-swarmifyx-plugin` starter template.
