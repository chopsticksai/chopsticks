# Abacus — Product Definition

## What It Is

Abacus is the control plane for autonomous AI companies. One instance of Abacus can run multiple companies. A **company** is a first-order object.
...
- **Adapter type + config** — how this agent runs and what defines its identity/behavior. This is adapter-specific (e.g., an OpenClaw agent might use SOUL.md and HEARTBEAT.md files; a Claude Code agent might use CLAUDE.md; a bare script might use CLI args). Abacus doesn't prescribe the format — the adapter does.
...
1. **Run a command** — Abacus kicks off a process (shell command, Python script, etc.) and tracks it. The heartbeat is "execute this and monitor it."
2. **Fire and forget a request** — Abacus sends a webhook/API call to an externally running agent. The heartbeat is "notify this agent to wake up." (OpenClaw hooks work this way.)
...
1. **Unopinionated about how you run your agents.** Your agents could be OpenClaw bots, Python scripts, Node scripts, Claude Code sessions, Codex instances — we don't care. Abacus defines the control plane for communication and provides utility infrastructure for heartbeats. It does not mandate an agent runtime.

2. **Company is the unit of organization.** Everything lives under a company. One Abacus instance, many companies.
...
5. **Control plane, not execution plane.** Abacus orchestrates. Agents run wherever they run and phone home.

## User Flow (Dream Scenario)

1. Open Abacus, create a new company
...
There are two runtime modes Abacus must support:
...
Abacus’s core identity is a **control plane for autonomous AI companies**, centered on **companies, org charts, goals, issues/comments, heartbeats, budgets, approvals, and board governance**. The public docs are also explicit about the current boundaries: **tasks/comments are the built-in communication model**, Abacus is **not a chatbot**, and it is **not a code review tool**. The roadmap already points toward **easier onboarding, cloud agents, easier agent configuration, plugins, better docs, and an AbacusHub marketplace for reusable companies/templates**.

## What Abacus should do vs. not do
...
- Do not build a complete Jira/GitHub replacement. The repo/docs already position Abacus as organization orchestration, not focused on pull-request review.
