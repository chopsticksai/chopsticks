...
This spec defines how Swarmifyx actually runs agents while staying runtime-agnostic.
...
1. Swarmifyx is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Swarmifyx manages lifecycle, and on exit Swarmifyx parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `swarmifyx.skill` (shared Swarmifyx skill text block)
...
2. Preferred transport is env vars (`SWARMIFYX_*`) injected at runtime.
