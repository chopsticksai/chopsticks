...
This spec defines how Chopsticks actually runs agents while staying runtime-agnostic.
...
1. Chopsticks is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Chopsticks manages lifecycle, and on exit Chopsticks parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `chopsticks.skill` (shared Chopsticks skill text block)
...
2. Preferred transport is env vars (`CHOPSTICKS_*`) injected at runtime.
