...
This document summarizes the memory systems referenced in task `PAP-530` and extracts the design patterns that matter for runeach.

## What runeach Needs From This Survey

runeach is not trying to become a single opinionated memory engine. The more useful target is a control-plane memory surface that:

- stays company-scoped
- lets each company choose a default memory provider
- lets specific agents override that default
- keeps provenance back to runeach runs, issues, comments, and documents
...
The question is not "which memory project wins?" The question is "what is the smallest runeach contract that can sit above several very different memory systems without flattening away the useful differences?"
...
These emphasize local persistence, inspectability, and low operational overhead. They are useful because runeach is local-first today and needs at least one zero-config path.

## Per-Project Notes

| Project | Shape | Notable API / model | Strong fit for runeach | Main mismatch |
|---|---|---|---|---|
...
| [mem0](https://github.com/mem0ai/mem0) | hosted + OSS SDK | `add`, `search`, `getAll`, `get`, `update`, `delete`, `deleteAll`; entity partitioning via `user_id`, `agent_id`, `run_id`, `app_id` | closest to a clean provider API with identities and metadata filters | provider owns extraction heavily; runeach should not assume every backend behaves like mem0 |
| [MemOS](https://github.com/MemTensor/MemOS) | memory OS / framework | unified add-retrieve-edit-delete, memory cubes, multimodal memory, tool memory, async scheduler, feedback/correction | strong source for optional capabilities beyond plain search | much broader than the minimal contract runeach should standardize first |
...
| [memU](https://github.com/NevaMind-AI/memU) | proactive agent memory framework | file-system metaphor, proactive loop, intent prediction, always-on companion model | good source for when memory should trigger agent behavior, not just retrieval | proactive assistant framing is broader than runeach's task-centric control plane |
| [Memori](https://github.com/MemoriLabs/Memori) | hosted memory fabric + SDK wrappers | registers against LLM SDKs, attribution via `entity_id` + `process_id`, sessions, cloud + BYODB | strong example of automatic capture around model clients | wrapper-centric design does not map 1:1 to runeach's run / issue / comment lifecycle |
...
If runeach does not expose these, it will not adapt well to the systems above.

## Where The Systems Differ

These differences are exactly why runeach needs a layered contract instead of a single hard-coded engine.
...
runeach should support both:

- provider-managed extraction
- runeach-managed extraction with provider-managed storage / retrieval
...
runeach should not require a single storage shape. It should require normalized references back to runeach entities.
...
runeach should make plain search the minimum contract and richer outputs optional capabilities.
...
runeach needs both direct request/response operations and background maintenance hooks.

## runeach-Specific Takeaways

### runeach should own these concerns

- binding a provider to a company and optionally overriding it per agent
- mapping runeach entities into provider scopes
- provenance back to issue comments, documents, runs, and activity
- cost / token / latency reporting for memory work
- browse and inspect surfaces in the runeach UI
...
runeach does not need to standardize every feature from every provider. It needs:
...
runeach should adopt a two-layer memory model:

1. `Memory binding + control plane layer`
   runeach decides which provider key is in effect for a company, agent, or project, and it logs every memory operation with provenance and usage.

2. `Provider adapter layer`
   A built-in or plugin-supplied adapter turns runeach memory requests into provider-specific calls.
...
without forcing runeach itself to become a monolithic memory engine.
