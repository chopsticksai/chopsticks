...
This document summarizes the memory systems referenced in task `PAP-530` and extracts the design patterns that matter for abacus.

## What abacus Needs From This Survey

abacus is not trying to become a single opinionated memory engine. The more useful target is a control-plane memory surface that:

- stays company-scoped
- lets each company choose a default memory provider
- lets specific agents override that default
- keeps provenance back to abacus runs, issues, comments, and documents
...
The question is not "which memory project wins?" The question is "what is the smallest abacus contract that can sit above several very different memory systems without flattening away the useful differences?"
...
These emphasize local persistence, inspectability, and low operational overhead. They are useful because abacus is local-first today and needs at least one zero-config path.

## Per-Project Notes

| Project | Shape | Notable API / model | Strong fit for abacus | Main mismatch |
|---|---|---|---|---|
...
| [mem0](https://github.com/mem0ai/mem0) | hosted + OSS SDK | `add`, `search`, `getAll`, `get`, `update`, `delete`, `deleteAll`; entity partitioning via `user_id`, `agent_id`, `run_id`, `app_id` | closest to a clean provider API with identities and metadata filters | provider owns extraction heavily; abacus should not assume every backend behaves like mem0 |
| [MemOS](https://github.com/MemTensor/MemOS) | memory OS / framework | unified add-retrieve-edit-delete, memory cubes, multimodal memory, tool memory, async scheduler, feedback/correction | strong source for optional capabilities beyond plain search | much broader than the minimal contract abacus should standardize first |
...
| [memU](https://github.com/NevaMind-AI/memU) | proactive agent memory framework | file-system metaphor, proactive loop, intent prediction, always-on companion model | good source for when memory should trigger agent behavior, not just retrieval | proactive assistant framing is broader than abacus's task-centric control plane |
| [Memori](https://github.com/MemoriLabs/Memori) | hosted memory fabric + SDK wrappers | registers against LLM SDKs, attribution via `entity_id` + `process_id`, sessions, cloud + BYODB | strong example of automatic capture around model clients | wrapper-centric design does not map 1:1 to abacus's run / issue / comment lifecycle |
...
If abacus does not expose these, it will not adapt well to the systems above.

## Where The Systems Differ

These differences are exactly why abacus needs a layered contract instead of a single hard-coded engine.
...
abacus should support both:

- provider-managed extraction
- abacus-managed extraction with provider-managed storage / retrieval
...
abacus should not require a single storage shape. It should require normalized references back to abacus entities.
...
abacus should make plain search the minimum contract and richer outputs optional capabilities.
...
abacus needs both direct request/response operations and background maintenance hooks.

## abacus-Specific Takeaways

### abacus should own these concerns

- binding a provider to a company and optionally overriding it per agent
- mapping abacus entities into provider scopes
- provenance back to issue comments, documents, runs, and activity
- cost / token / latency reporting for memory work
- browse and inspect surfaces in the abacus UI
...
abacus does not need to standardize every feature from every provider. It needs:
...
abacus should adopt a two-layer memory model:

1. `Memory binding + control plane layer`
   abacus decides which provider key is in effect for a company, agent, or project, and it logs every memory operation with provenance and usage.

2. `Provider adapter layer`
   A built-in or plugin-supplied adapter turns abacus memory requests into provider-specific calls.
...
without forcing abacus itself to become a monolithic memory engine.
