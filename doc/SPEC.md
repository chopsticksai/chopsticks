# Abacus Specification

Target specification for the Abacus control plane. Living document — updated incrementally during spec interviews.

---

## 1. Company Model [DRAFT]

A Company is a first-order object. One Abacus instance runs multiple Companies. A Company does not have a standalone "goal" field — its direction is defined by its set of Initiatives (see Task Hierarchy Mapping).
...
Concepts like SOUL.md (identity/mission) and HEARTBEAT.md (loop definition) are **not part of the Abacus protocol**. They are adapter-specific configurations. For example, an OpenClaw adapter might use SOUL.md and HEARTBEAT.md files. A Claude Code adapter might use CLAUDE.md. A bare Python script might use command-line args.

Abacus doesn't prescribe how an agent defines its identity or behavior. It provides the control plane; the adapter defines the agent's inner workings.
...
#### Abacus Protocol (What Abacus Knows)

At the protocol level, Abacus tracks:
...
- **Fat payload** — Abacus bundles relevant context (current tasks, messages, company state, metrics) into the heartbeat invocation. Suited for simple/stateless agents that can't call back to Abacus.
- **Thin ping** — Heartbeat is just a wake-up signal. Agent calls Abacus's API to fetch whatever context it needs. Suited for sophisticated agents that manage their own state.

#### Minimum Contract

The minimum requirement to be a Abacus agent: **be callable.** That's it. Abacus can invoke you via command or webhook. No requirement to report back — Abacus infers basic status from process liveness when it can.

#### Integration Levels

Beyond the minimum, Abacus provides progressively richer integration:

1. **Callable** (minimum) — Abacus can start you. That's the only contract.
...
Abacus ships **default agents** that demonstrate full integration: progress tracking, cost instrumentation, and a **Abacus skill** (a Claude Code skill for interacting with the Abacus API) for task management. These serve as both useful defaults and reference implementations for adapter authors.
...
Agents can create tasks and assign them to agents outside their reporting line. This is the mechanism for cross-team collaboration. These rules are primarily encoded in the Abacus SKILL.md which is recommended for all agents. Abacus the app enforces the tooling and some light governance, but the cross-team rules below are mainly implemented by agent decisions.
...
The heartbeat is a protocol, not a runtime. Abacus defines how to initiate an agent's cycle. What the agent does with that cycle — how long it runs, whether it's task-scoped or continuous — is entirely up to the agent.

### Execution Adapters

Agent configuration includes an **adapter** that defines how Abacus invokes the agent. Initial adapters:
...
This is the full adapter contract. `invoke` starts the agent, `status` lets Abacus check on it, `cancel` enables the board's pause functionality. Everything else (cost reporting, task updates) is optional and flows through the Abacus REST API.

### What Abacus Controls
...
- Health monitoring — how does Abacus distinguish "stuck" from "working on a long task"?
...
Token/LLM cost budgeting is a core part of Abacus. External revenue and expense tracking is a future plugin.

### Cost Reporting

Fully-instrumented Agents report token/API usage back to Abacus. Costs are tracked at every level:
...
Abacus ships default Agent templates:

- **Default Agent** — a basic Claude Code or Codex loop. Knows the **Abacus Skill** (SKILL.md) so it can interact with the task system, read Company context, report status.
...
### Abacus Skill (SKILL.md)

A skill definition that teaches agents how to interact with Abacus. Provides:
...
The key constraint: it must be trivial to go from "I'm trying this on my machine" to "my agents are running on remote servers talking to my Abacus instance."

#### Agent Authentication

When a user creates an Agent, Abacus generates a **connection string** containing: the server URL, an API key, and instructions for how to authenticate. The Agent is assumed to be capable of figuring out how to call the API with its token/key from there.

Flow:

1. Human creates an Agent in the UI
2. Abacus generates a connection string (URL + key + instructions)
...
When a human completes a task, if the requesting agent's adapter supports **pingbacks** (e.g. OpenClaw hooks), Abacus sends a notification to wake that agent. This keeps humans rare but possible participants in the workflow.

The agents are discouraged from assigning tasks to humans in the Abacus SKILL, but sometimes it's unavoidable.
...
Abacus does **not** manage work artifacts (code repos, file systems, deployments, documents). That's entirely the agent's domain. Abacus tracks tasks and costs. Where and how work gets done is outside scope.
...
When an agent crashes or disappears mid-task, Abacus does **not** auto-reassign or auto-release the task. Instead:

- Abacus surfaces stale tasks (tasks in `in_progress` with no recent activity) through dashboards and reporting
- Abacus does not fail silently — the auditing and visibility tools make problems obvious
...
**Principle: Abacus reports problems, it doesn't silently fix them.** Automatic recovery hides failures. Good visibility lets the right entity (human or agent) decide what to do.

### Plugin / Extension Architecture

The core Abacus system must be extensible. Features like knowledge bases, external revenue tracking, and new Agent Adapters should be addable as **plugins** without modifying core. This means:
...
The plugin framework has shipped. Plugins can register new adapter types, hook into lifecycle events, and contribute UI components (e.g. global toolbar buttons). A plugin SDK and CLI commands (`abacus plugin`) are available for authoring and installing plugins.
...
**Full loop with one adapter.** V1 must demonstrate the complete Abacus cycle end-to-end, even if narrow.
...
- [ ] **Default agent** — basic Claude Code/Codex loop with Abacus skill
- [ ] **Default CEO** — strategic planning, delegation, board communication
- [ ] **Abacus skill (SKILL.md)** — teaches agents to interact with the API
...
**Anti-goal for core.** The knowledge base is not part of the Abacus core — it will be a plugin. The task system + comments + agent descriptions provide sufficient shared context.
...
Things Abacus explicitly does **not** do:

- **Not an Agent runtime** — Abacus orchestrates, Agents run elsewhere
...
1. **Unopinionated about how you run your Agents.** Any language, any framework, any runtime. Abacus is the control plane, not the execution plane.
