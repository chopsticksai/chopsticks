# RunEach Specification

Target specification for the RunEach control plane. Living document — updated incrementally during spec interviews.

---

## 1. Company Model [DRAFT]

A Company is a first-order object. One RunEach instance runs multiple Companies. A Company does not have a standalone "goal" field — its direction is defined by its set of Initiatives (see Task Hierarchy Mapping).
...
Concepts like SOUL.md (identity/mission) and HEARTBEAT.md (loop definition) are **not part of the RunEach protocol**. They are adapter-specific configurations. For example, an OpenClaw adapter might use SOUL.md and HEARTBEAT.md files. A Claude Code adapter might use CLAUDE.md. A bare Python script might use command-line args.

RunEach doesn't prescribe how an agent defines its identity or behavior. It provides the control plane; the adapter defines the agent's inner workings.
...
#### RunEach Protocol (What RunEach Knows)

At the protocol level, RunEach tracks:
...
- **Fat payload** — RunEach bundles relevant context (current tasks, messages, company state, metrics) into the heartbeat invocation. Suited for simple/stateless agents that can't call back to RunEach.
- **Thin ping** — Heartbeat is just a wake-up signal. Agent calls RunEach's API to fetch whatever context it needs. Suited for sophisticated agents that manage their own state.

#### Minimum Contract

The minimum requirement to be a RunEach agent: **be callable.** That's it. RunEach can invoke you via command or webhook. No requirement to report back — RunEach infers basic status from process liveness when it can.

#### Integration Levels

Beyond the minimum, RunEach provides progressively richer integration:

1. **Callable** (minimum) — RunEach can start you. That's the only contract.
...
RunEach ships **default agents** that demonstrate full integration: progress tracking, cost instrumentation, and a **RunEach skill** (a Claude Code skill for interacting with the RunEach API) for task management. These serve as both useful defaults and reference implementations for adapter authors.
...
Agents can create tasks and assign them to agents outside their reporting line. This is the mechanism for cross-team collaboration. These rules are primarily encoded in the RunEach SKILL.md which is recommended for all agents. RunEach the app enforces the tooling and some light governance, but the cross-team rules below are mainly implemented by agent decisions.
...
The heartbeat is a protocol, not a runtime. RunEach defines how to initiate an agent's cycle. What the agent does with that cycle — how long it runs, whether it's task-scoped or continuous — is entirely up to the agent.

### Execution Adapters

Agent configuration includes an **adapter** that defines how RunEach invokes the agent. Initial adapters:
...
This is the full adapter contract. `invoke` starts the agent, `status` lets RunEach check on it, `cancel` enables the board's pause functionality. Everything else (cost reporting, task updates) is optional and flows through the RunEach REST API.

### What RunEach Controls
...
- Health monitoring — how does RunEach distinguish "stuck" from "working on a long task"?
...
Token/LLM cost budgeting is a core part of RunEach. External revenue and expense tracking is a future plugin.

### Cost Reporting

Fully-instrumented Agents report token/API usage back to RunEach. Costs are tracked at every level:
...
RunEach ships default Agent templates:

- **Default Agent** — a basic Claude Code or Codex loop. Knows the **RunEach Skill** (SKILL.md) so it can interact with the task system, read Company context, report status.
...
### RunEach Skill (SKILL.md)

A skill definition that teaches agents how to interact with RunEach. Provides:
...
The key constraint: it must be trivial to go from "I'm trying this on my machine" to "my agents are running on remote servers talking to my RunEach instance."

#### Agent Authentication

When a user creates an Agent, RunEach generates a **connection string** containing: the server URL, an API key, and instructions for how to authenticate. The Agent is assumed to be capable of figuring out how to call the API with its token/key from there.

Flow:

1. Human creates an Agent in the UI
2. RunEach generates a connection string (URL + key + instructions)
...
When a human completes a task, if the requesting agent's adapter supports **pingbacks** (e.g. OpenClaw hooks), RunEach sends a notification to wake that agent. This keeps humans rare but possible participants in the workflow.

The agents are discouraged from assigning tasks to humans in the RunEach SKILL, but sometimes it's unavoidable.
...
RunEach does **not** manage work artifacts (code repos, file systems, deployments, documents). That's entirely the agent's domain. RunEach tracks tasks and costs. Where and how work gets done is outside scope.
...
When an agent crashes or disappears mid-task, RunEach does **not** auto-reassign or auto-release the task. Instead:

- RunEach surfaces stale tasks (tasks in `in_progress` with no recent activity) through dashboards and reporting
- RunEach does not fail silently — the auditing and visibility tools make problems obvious
...
**Principle: RunEach reports problems, it doesn't silently fix them.** Automatic recovery hides failures. Good visibility lets the right entity (human or agent) decide what to do.

### Plugin / Extension Architecture

The core RunEach system must be extensible. Features like knowledge bases, external revenue tracking, and new Agent Adapters should be addable as **plugins** without modifying core. This means:
...
The plugin framework has shipped. Plugins can register new adapter types, hook into lifecycle events, and contribute UI components (e.g. global toolbar buttons). A plugin SDK and CLI commands (`runeach plugin`) are available for authoring and installing plugins.
...
**Full loop with one adapter.** V1 must demonstrate the complete RunEach cycle end-to-end, even if narrow.
...
- [ ] **Default agent** — basic Claude Code/Codex loop with RunEach skill
- [ ] **Default CEO** — strategic planning, delegation, board communication
- [ ] **RunEach skill (SKILL.md)** — teaches agents to interact with the API
...
**Anti-goal for core.** The knowledge base is not part of the RunEach core — it will be a plugin. The task system + comments + agent descriptions provide sufficient shared context.
...
Things RunEach explicitly does **not** do:

- **Not an Agent runtime** — RunEach orchestrates, Agents run elsewhere
...
1. **Unopinionated about how you run your Agents.** Any language, any framework, any runtime. RunEach is the control plane, not the execution plane.
