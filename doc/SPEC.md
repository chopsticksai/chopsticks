# Chopsticks Specification

Target specification for the Chopsticks control plane. Living document — updated incrementally during spec interviews.

---

## 1. Company Model [DRAFT]

A Company is a first-order object. One Chopsticks instance runs multiple Companies. A Company does not have a standalone "goal" field — its direction is defined by its set of Initiatives (see Task Hierarchy Mapping).
...
Concepts like SOUL.md (identity/mission) and HEARTBEAT.md (loop definition) are **not part of the Chopsticks protocol**. They are adapter-specific configurations. For example, an OpenClaw adapter might use SOUL.md and HEARTBEAT.md files. A Claude Code adapter might use CLAUDE.md. A bare Python script might use command-line args.

Chopsticks doesn't prescribe how an agent defines its identity or behavior. It provides the control plane; the adapter defines the agent's inner workings.
...
#### Chopsticks Protocol (What Chopsticks Knows)

At the protocol level, Chopsticks tracks:
...
- **Fat payload** — Chopsticks bundles relevant context (current tasks, messages, company state, metrics) into the heartbeat invocation. Suited for simple/stateless agents that can't call back to Chopsticks.
- **Thin ping** — Heartbeat is just a wake-up signal. Agent calls Chopsticks's API to fetch whatever context it needs. Suited for sophisticated agents that manage their own state.

#### Minimum Contract

The minimum requirement to be a Chopsticks agent: **be callable.** That's it. Chopsticks can invoke you via command or webhook. No requirement to report back — Chopsticks infers basic status from process liveness when it can.

#### Integration Levels

Beyond the minimum, Chopsticks provides progressively richer integration:

1. **Callable** (minimum) — Chopsticks can start you. That's the only contract.
...
Chopsticks ships **default agents** that demonstrate full integration: progress tracking, cost instrumentation, and a **Chopsticks skill** (a Claude Code skill for interacting with the Chopsticks API) for task management. These serve as both useful defaults and reference implementations for adapter authors.
...
Agents can create tasks and assign them to agents outside their reporting line. This is the mechanism for cross-team collaboration. These rules are primarily encoded in the Chopsticks SKILL.md which is recommended for all agents. Chopsticks the app enforces the tooling and some light governance, but the cross-team rules below are mainly implemented by agent decisions.
...
The heartbeat is a protocol, not a runtime. Chopsticks defines how to initiate an agent's cycle. What the agent does with that cycle — how long it runs, whether it's task-scoped or continuous — is entirely up to the agent.

### Execution Adapters

Agent configuration includes an **adapter** that defines how Chopsticks invokes the agent. Initial adapters:
...
This is the full adapter contract. `invoke` starts the agent, `status` lets Chopsticks check on it, `cancel` enables the board's pause functionality. Everything else (cost reporting, task updates) is optional and flows through the Chopsticks REST API.

### What Chopsticks Controls
...
### What Chopsticks Does NOT Control
...
- Health monitoring — how does Chopsticks distinguish "stuck" from "working on a long task"?
...
Token/LLM cost budgeting is a core part of Chopsticks. External revenue and expense tracking is a future plugin.

### Cost Reporting

Fully-instrumented Agents report token/API usage back to Chopsticks. Costs are tracked at every level:
...
Chopsticks ships default Agent templates:

- **Default Agent** — a basic Claude Code or Codex loop. Knows the **Chopsticks Skill** (SKILL.md) so it can interact with the task system, read Company context, report status.
...
### Chopsticks Skill (SKILL.md)

A skill definition that teaches agents how to interact with Chopsticks. Provides:
...
The key constraint: it must be trivial to go from "I'm trying this on my machine" to "my agents are running on remote servers talking to my Chopsticks instance."

#### Agent Authentication

When a user creates an Agent, Chopsticks generates a **connection string** containing: the server URL, an API key, and instructions for how to authenticate. The Agent is assumed to be capable of figuring out how to call the API with its token/key from there.

Flow:

1. Human creates an Agent in the UI
2. Chopsticks generates a connection string (URL + key + instructions)
...
When a human completes a task, if the requesting agent's adapter supports **pingbacks** (e.g. OpenClaw hooks), Chopsticks sends a notification to wake that agent. This keeps humans rare but possible participants in the workflow.

The agents are discouraged from assigning tasks to humans in the Chopsticks SKILL, but sometimes it's unavoidable.
...
Chopsticks does **not** manage work artifacts (code repos, file systems, deployments, documents). That's entirely the agent's domain. Chopsticks tracks tasks and costs. Where and how work gets done is outside scope.
...
When an agent crashes or disappears mid-task, Chopsticks does **not** auto-reassign or auto-release the task. Instead:

- Chopsticks surfaces stale tasks (tasks in `in_progress` with no recent activity) through dashboards and reporting
- Chopsticks does not fail silently — the auditing and visibility tools make problems obvious
...
**Principle: Chopsticks reports problems, it doesn't silently fix them.** Automatic recovery hides failures. Good visibility lets the right entity (human or agent) decide what to do.

### Plugin / Extension Architecture

The core Chopsticks system must be extensible. Features like knowledge bases, external revenue tracking, and new Agent Adapters should be addable as **plugins** without modifying core. This means:
...
The plugin framework has shipped. Plugins can register new adapter types, hook into lifecycle events, and contribute UI components (e.g. global toolbar buttons). A plugin SDK and CLI commands (`chopsticks plugin`) are available for authoring and installing plugins.
...
**Full loop with one adapter.** V1 must demonstrate the complete Chopsticks cycle end-to-end, even if narrow.
...
- [ ] **Default agent** — basic Claude Code/Codex loop with Chopsticks skill
- [ ] **Default CEO** — strategic planning, delegation, board communication
- [ ] **Chopsticks skill (SKILL.md)** — teaches agents to interact with the API
...
**Anti-goal for core.** The knowledge base is not part of the Chopsticks core — it will be a plugin. The task system + comments + agent descriptions provide sufficient shared context.
...
Things Chopsticks explicitly does **not** do:

- **Not an Agent runtime** — Chopsticks orchestrates, Agents run elsewhere
...
1. **Unopinionated about how you run your Agents.** Any language, any framework, any runtime. Chopsticks is the control plane, not the execution plane.
