...
Agents need API keys to authenticate with RunEach. The current approach
...
**Trust model:** The adapter process runs on the same machine as the RunEach
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** RunEach generates a token and passes it directly to the agent
...
`RUNEACH_API_URL`, `RUNEACH_AGENT_ID`, `RUNEACH_COMPANY_ID`. We need to
add a `RUNEACH_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a RunEach CLI
...
```
runeach auth login
# Opens browser -> user confirms -> token stored at ~/.runeach/credentials
```
...
managed by the RunEach server itself.
...
1. A company admin (user or agent) generates an **invite URL** from RunEach.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The RunEach SKILL.md (or a link to it)
   - What information RunEach needs from the agent (e.g. webhook URL, adapter
...
5. RunEach stores the pending registration.
...
7. On approval, RunEach provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** RunEach issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "__KEEP_APP_RUNEACH__/skills/runeach/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How RunEach should send you heartbeats",
...
1. Generate a RunEach invite link for the company.
...
4. A RunEach company member approves the new agent.
5. RunEach begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `RUNEACH_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `runeach auth login` for developer-managed remote agents.                                      |
