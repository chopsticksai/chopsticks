...
Swamifyx needs to support two very different but equally valid ways of working:
...
Today, Swamifyx already has the beginnings of this model:
...
- making Swamifyx feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Swamifyx users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Swamifyx deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Swamifyx into a full code review product
...
This object must be recorded explicitly so that Swamifyx can:
...
Swamifyx should treat PRs as a type of work product linked back to:
...
Swamifyx already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Swamifyx must be a first-class deployment mode

Swamifyx cannot assume that it is running on the same machine as the code.

In cloud deployments, Swamifyx may:
...
- `Runtime service`: a process or service Swamifyx owns or tracks for a workspace
...
It also lets cloud-hosted Swamifyx deployments point at codebases and remotes without pretending the Swamifyx host has direct filesystem access.
...
Without an explicit `execution workspace` record, Swamifyx has nowhere stable to attach:
...
Swamifyx needs a single place to show:
...
- Swamifyx-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Swamifyx owning the execution host
...
## 1. Swamifyx host and execution host must be decoupled
...
without the Swamifyx host itself running local git or local preview processes.
...
That keeps Swamifyx focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Swamifyx deployments remain first-class

That is a better fit for Swamifyx than either extreme:
