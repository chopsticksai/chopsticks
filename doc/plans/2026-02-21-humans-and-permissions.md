...
- local mode that still feels instant (`npx swarmifyx run` and go)
...
2. CLI command `pnpm swarmifyx auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm swarmifyx onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm swarmifyx onboard`).
...
- expose key instance settings in API + CLI (`swarmifyx configure` / `swarmifyx onboard`)
...
7. `pnpm swarmifyx onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
