...
- local mode that still feels instant (`npx runeach run` and go)
...
2. CLI command `pnpm runeach auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm runeach onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm runeach onboard`).
...
- expose key instance settings in API + CLI (`runeach configure` / `runeach onboard`)
...
7. `pnpm runeach onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
