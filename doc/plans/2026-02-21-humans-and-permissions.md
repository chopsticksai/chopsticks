...
- local mode that still feels instant (`npx chopsticks run` and go)
...
2. CLI command `pnpm chopsticks auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm chopsticks onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm chopsticks onboard`).
...
- expose key instance settings in API + CLI (`chopsticks configure` / `chopsticks onboard`)
...
7. `pnpm chopsticks onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
