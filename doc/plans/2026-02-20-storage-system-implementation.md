...
Add a single storage subsystem for Abacus that supports:
...
- API serves attachment bytes through authenticated Abacus endpoints.
...
- Default local path is under instance root: `~/.abacus/instances/<instanceId>/data/storage`.
...
- `abacus onboard` writes a valid `storage` config block by default.
- `abacus configure --section storage` can switch between local and s3 modes.
...
- `abacus doctor` reports actionable storage status.
