...
Add a single storage subsystem for Swarmifyx that supports:
...
- API serves attachment bytes through authenticated Swarmifyx endpoints.
...
- Default local path is under instance root: `~/.swarmifyx/instances/<instanceId>/data/storage`.
...
- `swarmifyx onboard` writes a valid `storage` config block by default.
- `swarmifyx configure --section storage` can switch between local and s3 modes.
...
- `swarmifyx doctor` reports actionable storage status.
