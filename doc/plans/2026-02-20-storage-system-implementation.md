...
Add a single storage subsystem for RunEach that supports:
...
- API serves attachment bytes through authenticated RunEach endpoints.
...
- Default local path is under instance root: `~/.runeach/instances/<instanceId>/data/storage`.
...
- `runeach onboard` writes a valid `storage` config block by default.
- `runeach configure --section storage` can switch between local and s3 modes.
...
- `runeach doctor` reports actionable storage status.
