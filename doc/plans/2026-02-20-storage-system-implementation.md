...
Add a single storage subsystem for Chopsticks that supports:
...
- API serves attachment bytes through authenticated Chopsticks endpoints.
...
- Default local path is under instance root: `~/.chopsticks/instances/<instanceId>/data/storage`.
...
- `chopsticks onboard` writes a valid `storage` config block by default.
- `chopsticks configure --section storage` can switch between local and s3 modes.
...
- `chopsticks doctor` reports actionable storage status.
