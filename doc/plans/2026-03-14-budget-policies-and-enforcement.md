...
abacus already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model abacus should implement next.

## Product Goals

abacus should let operators:
...
For the next implementation pass, abacus should enforce these defaults:
...
abacus should separate subscription-included usage from billed spend:
...
abacus should have two threshold classes:
...
abacus must know that a project is budget-paused and enforce it.
...
abacus must also block execution before new work starts.
