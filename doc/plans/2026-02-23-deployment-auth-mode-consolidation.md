...
Keep Chopsticks low-friction while making the mode model simpler and safer:
...
1. `pnpm chopsticks onboard` is interactive-first and defaults to `local_trusted`.
2. authenticated mode is one runtime mode with `private/public` exposure guidance.
3. `pnpm chopsticks doctor` works flagless with mode-aware checks.
