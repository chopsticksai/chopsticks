...
This document defines the V1 design for a Chopsticks adapter that integrates with
...
4. synthesized stdout events for Chopsticks UI/CLI
...
3. **Agent callback auth to Chopsticks** uses a bootstrap exchange flow (no long-lived Chopsticks key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Chopsticks endpoints, not full SKILL.md prompt injection.
...
- `chopsticksPublicUrl` (optional override; else `CHOPSTICKS_PUBLIC_URL` env)
...
## Chopsticks Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `CHOPSTICKS_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.chopsticksPublicUrl`
2. `process.env.CHOPSTICKS_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Chopsticks credentials in prompt text.

Flow:

1. Before launch/follow-up, Chopsticks mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Chopsticks validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Chopsticks API calls.
...
1. Prompt includes a compact instruction to fetch skills from Chopsticks.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/chopsticks`
   - `GET /api/skills/chopsticks-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <chopsticksPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Chopsticks run context.
...
5. `chopsticksPublicUrl` present and reachable shape-valid
...
- poll interval
- timeout/grace
- chopsticks public URL override
- enable webhooks
...
| Skill delivery | local skill dir injection | authenticated fetch from Chopsticks skill endpoints |
| Chopsticks auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Chopsticks context.
3. Add richer PR metadata surfacing in Chopsticks UI.
