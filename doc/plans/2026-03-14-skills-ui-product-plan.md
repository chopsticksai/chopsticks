...
This document defines the product and UI plan for skill management in RunEach.
...
- RunEach company import/export can include skills as package content
...
3. The UI must always tell the truth about what RunEach knows:
   - desired state in RunEach
...
RunEach should treat skills at two scopes:
...
- created locally inside RunEach later
...
7. “Understand what RunEach knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a RunEach-specific package layout
...
This matters because Codex and similar adapters may already have local skills that RunEach did not install.

These should be clearly marked:

- external
- not managed by RunEach
...
- `desired_only`: RunEach wants it, adapter does not show it yet
- `external`: adapter has it but RunEach does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, RunEach only tracks desired state
...
- RunEach can still track desired skills
...
- RunEach can see adapter skills
...
- `.runeach.yaml` may add RunEach-specific fidelity, but should not replace the base shortname association model
...
- `.runeach.yaml` may add RunEach-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
RunEach should have a company-scoped skill model or managed package model representing:
...
RunEach should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too RunEach-specific.
...
That gives RunEach one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
