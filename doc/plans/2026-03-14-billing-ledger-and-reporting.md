...
abacus currently stores model spend in `cost_events` and operational run state in `heartbeat_runs`.
...
However, if abacus needs to account for the full billing surface of aggregators and managed AI platforms, then `cost_events` alone is not enough.
...
It follows directly from the billing systems of providers and aggregators abacus needs to support.
