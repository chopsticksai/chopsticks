# Abacus Module System

## Overview

Abacus's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Abacus's API, UI, and data model | Yes |
...
modules/
  observability/
    abacus.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@abacus/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@abacus/core";
...
```
1. Scan modules/ directory for abacus.module.json manifests
...
```jsonc
// abacus.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@abacus/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@abacus/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:abacus/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "__KEEP_STORE_ABACUS__/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm abacus store list                    # browse available modules and templates
pnpm abacus store install <module-id>     # install a module
pnpm abacus store import <template-id>    # import a company template
pnpm abacus store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Abacus instance | `server:started` |
...
4. **Module config** — `abacus.config.json` with per-module config, env var interpolation
...
10. **`@abacus/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
