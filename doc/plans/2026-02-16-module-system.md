...
RunEach's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends RunEach's API, UI, and data model | Yes |
...
modules/
  observability/
    runeach.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@runeach/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@runeach/core";
...
import type { Db } from "@runeach/db";
...
```
1. Scan modules/ directory for runeach.module.json manifests
...
```jsonc
// runeach.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@runeach/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@runeach/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:runeach/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "__KEEP_STORE_RUNEACH__/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm runeach store list                    # browse available modules and templates
pnpm runeach store install <module-id>     # install a module
pnpm runeach store import <template-id>    # import a company template
pnpm runeach store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one RunEach instance | `server:started` |
...
4. **Module config** — `runeach.config.json` with per-module config, env var interpolation
...
10. **`@runeach/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
