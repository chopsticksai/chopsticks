...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/runeach-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `RUNEACH_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `RUNEACH_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - runeach may create host-local worktrees, processes, and paths
- `cloud`
  - runeach should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `runeach | github | vercel | s3 | custom`
...
- use `RUNEACH_EXECUTION_TOPOLOGY` as a defaulting guardrail
