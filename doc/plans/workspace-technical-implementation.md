...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/abacus-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `ABACUS_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `ABACUS_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - abacus may create host-local worktrees, processes, and paths
- `cloud`
  - abacus should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `abacus | github | vercel | s3 | custom`
...
- use `ABACUS_EXECUTION_TOPOLOGY` as a defaulting guardrail
