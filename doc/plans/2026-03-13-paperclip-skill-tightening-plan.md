# Swamifyx Skill Tightening Plan
...
The `chopsticks` skill is part of the critical control-plane safety surface. Tightening it may reduce fresh-session token use, but it also carries prompt-regression risk. We do not yet have evals that would let us safely prove behavior preservation across assignment handling, checkout rules, comment etiquette, approval workflows, and escalation paths.
...
Fresh runs still spend substantial input tokens even after the context-path fixes. The remaining large startup cost appears to come from loading the full `chopsticks` skill and related instruction surface into context at run start.
...
- preserve all current Swamifyx control-plane capabilities
...
- changing Swamifyx API semantics
...
- add at least lightweight eval or scripted scenario coverage for key Swamifyx flows
- confirm how adapter/bootstrap layering should load skill content versus references

## Success Criteria

- materially lower first-run input tokens for Swamifyx-coordinated agents
