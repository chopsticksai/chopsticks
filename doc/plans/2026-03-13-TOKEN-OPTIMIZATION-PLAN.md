...
Date: 2026-03-13  
Related discussion: https://github.com/runeachai/runeach/discussions/449
...
- local runtime data from the default Swamifyx instance on 2026-03-13
...
3. **Repeated context reacquisition.** The `runeach` skill tells agents to re-fetch assignments, issue details, ancestors, and full comment threads on every heartbeat. The API does not currently offer efficient delta-oriented alternatives.
...
The `runeach` skill currently tells agents to do this on essentially every heartbeat:
...
- Swamifyx discovers repo skills from the current checkout, then symlinks them into `$CODEX_HOME/skills` or `~/.codex/skills`.
- If an existing Swamifyx skill symlink already points at another live checkout, the current implementation skips it instead of repointing it.
- This can leave Codex using stale skill content from a different worktree even after Swamifyx-side skill changes land.
...
- `skills/runeach/SKILL.md`: 17,441 bytes
...
- Store both:
  ...
  - Swamifyx-normalized per-run usage
...
For `codex_local`, this also requires isolating the Codex skill home per worktree or teaching Swamifyx to repoint its own skill symlinks when the source checkout changes. Otherwise prompt and skill improvements in the active worktree may not reach the running agent.
...
Update the `runeach` skill so the default pattern becomes:
...
- For `codex_local`, either:
  ...
  - treat Swamifyx-owned Codex skill symlinks as repairable when they point at a different checkout
...
4. heartbeat delta APIs + `runeach` skill rewrite
...
6. Rewrite `skills/runeach/SKILL.md` around delta-fetch behavior.
