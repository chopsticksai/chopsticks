...
3. **Repeated context reacquisition.** The `chopsticks` skill tells agents to re-fetch assignments, issue details, ancestors, and full comment threads on every heartbeat. The API does not currently offer efficient delta-oriented alternatives.
...
The `chopsticks` skill currently tells agents to do this on essentially every heartbeat:
...
- `skills/chopsticks/SKILL.md`: 17,441 bytes
...
Update the `chopsticks` skill so the default pattern becomes:
...
4. heartbeat delta APIs + `chopsticks` skill rewrite
...
6. Rewrite `skills/chopsticks/SKILL.md` around delta-fetch behavior.
