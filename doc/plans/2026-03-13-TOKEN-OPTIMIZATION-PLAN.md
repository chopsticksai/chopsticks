...
Date: 2026-03-13  
Related discussion: https://github.com/abacus-lab/abacus/discussions/449
...
3. **Repeated context reacquisition.** The `abacus` skill tells agents to re-fetch assignments, issue details, ancestors, and full comment threads on every heartbeat. The API does not currently offer efficient delta-oriented alternatives.
...
The `abacus` skill currently tells agents to do this on essentially every heartbeat:
...
- `skills/abacus/SKILL.md`: 17,441 bytes
...
Update the `abacus` skill so the default pattern becomes:
...
4. heartbeat delta APIs + `abacus` skill rewrite
...
6. Rewrite `skills/abacus/SKILL.md` around delta-fetch behavior.
