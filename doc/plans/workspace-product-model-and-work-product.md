...
abacus needs to support two very different but equally valid ways of working:
...
Today, abacus already has the beginnings of this model:
...
- making abacus feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing abacus users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted abacus deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning abacus into a full code review product
...
This object must be recorded explicitly so that abacus can:
...
abacus should treat PRs as a type of work product linked back to:
...
abacus already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted abacus must be a first-class deployment mode

abacus cannot assume that it is running on the same machine as the code.

In cloud deployments, abacus may:
...
- `Runtime service`: a process or service abacus owns or tracks for a workspace
...
It also lets cloud-hosted abacus deployments point at codebases and remotes without pretending the abacus host has direct filesystem access.
...
Without an explicit `execution workspace` record, abacus has nowhere stable to attach:
...
abacus needs a single place to show:
...
- abacus-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without abacus owning the execution host
...
## 1. abacus host and execution host must be decoupled
...
without the abacus host itself running local git or local preview processes.
...
abacus should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for abacus.
...
abacus should keep workspace automation centered on a small set of hooks:
...
abacus should provide the equivalent visibility in the project `Code` surface:
...
abacus should treat execution workspaces as the natural home for:
...
abacus should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
abacus should continue treating cleanup policy as part of the core workspace model:
...
abacus should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the abacus host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

abacus should borrow Worktrunk's good execution patterns while keeping the broader abacus model:
...
That keeps abacus focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted abacus deployments remain first-class

That is a better fit for abacus than either extreme:
