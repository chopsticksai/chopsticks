...
runeach needs to support two very different but equally valid ways of working:
...
Today, runeach already has the beginnings of this model:
...
- making runeach feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing runeach users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted runeach deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning runeach into a full code review product
...
This object must be recorded explicitly so that runeach can:
...
runeach should treat PRs as a type of work product linked back to:
...
runeach already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted runeach must be a first-class deployment mode

runeach cannot assume that it is running on the same machine as the code.

In cloud deployments, runeach may:
...
- `Runtime service`: a process or service runeach owns or tracks for a workspace
...
It also lets cloud-hosted runeach deployments point at codebases and remotes without pretending the runeach host has direct filesystem access.
...
Without an explicit `execution workspace` record, runeach has nowhere stable to attach:
...
runeach needs a single place to show:
...
- runeach-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without runeach owning the execution host
...
## 1. runeach host and execution host must be decoupled
...
without the runeach host itself running local git or local preview processes.
...
runeach should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for runeach.
...
runeach should keep workspace automation centered on a small set of hooks:
...
runeach should provide the equivalent visibility in the project `Code` surface:
...
runeach should treat execution workspaces as the natural home for:
...
runeach should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
runeach should continue treating cleanup policy as part of the core workspace model:
...
runeach should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the runeach host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

runeach should borrow Worktrunk's good execution patterns while keeping the broader runeach model:
...
That keeps runeach focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted runeach deployments remain first-class

That is a better fit for runeach than either extreme:
