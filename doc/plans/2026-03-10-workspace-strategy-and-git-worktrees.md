...
`PAP-447` asks how Chopsticks should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Chopsticks instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Chopsticks:

- some operators use Chopsticks to manage Chopsticks and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Chopsticks
- Claude and Codex expose different built-in affordances, so Chopsticks should not overfit to one tool

## Core Product Decision

Chopsticks should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Chopsticks
...
- Chopsticks should not assume that every adapter can see or use a host filesystem path directly
...
If Chopsticks makes worktrees agent-first, it will blur:
...
- Chopsticks core resolves and records execution workspace state
...
That gives Chopsticks room to support:
...
- Chopsticks handles workspace runtime services under the hood using project/workspace policy
...
Once Chopsticks is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Chopsticks should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Chopsticks cannot infer
...
- when issue moves to done, should Chopsticks auto-commit?
- should it auto-push?
...
Chopsticks already has the right foundation for a project-first model.
...
For server-spawned local adapters, Chopsticks already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Chopsticks should mirror directly

Implication:

- for `codex_local`, Chopsticks should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Chopsticks should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Chopsticks should separate:
...
Chopsticks should be able to express intentions such as:
...
These are not specific to Chopsticks. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".chopsticks/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Chopsticks-specific `devServer` flag.
...
Chopsticks should distinguish between:
...
Chopsticks should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Chopsticks should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Chopsticks
...
For runtime services specifically, Chopsticks should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Chopsticks-only dev-server flags
...
3. Inject resolved execution workspace details into `context.chopsticksWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Chopsticks instance can be reused through env/config injection when appropriate
...
- Chopsticks can decide whether to reuse or start a fresh service deterministically
...
3. Test against Chopsticks-on-Chopsticks workflows first.
...
7. Workspace runtime services are modeled generically, not as Chopsticks-specific dev-server toggles.
