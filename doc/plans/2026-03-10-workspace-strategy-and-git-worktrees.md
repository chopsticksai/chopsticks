...
`PAP-447` asks how Abacus should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Abacus instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Abacus:

- some operators use Abacus to manage Abacus and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Abacus
- Claude and Codex expose different built-in affordances, so Abacus should not overfit to one tool

## Core Product Decision

Abacus should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Abacus
...
- Abacus should not assume that every adapter can see or use a host filesystem path directly
...
If Abacus makes worktrees agent-first, it will blur:
...
- Abacus core resolves and records execution workspace state
...
That gives Abacus room to support:
...
- Abacus handles workspace runtime services under the hood using project/workspace policy
...
Once Abacus is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Abacus cannot infer
...
- when issue moves to done, should Abacus auto-commit?
- should it auto-push?
...
Abacus already has the right foundation for a project-first model.
...
For server-spawned local adapters, Abacus already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Abacus should mirror directly

Implication:

- for `codex_local`, Abacus should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Abacus should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Abacus should separate:
...
Abacus should be able to express intentions such as:
...
These are not specific to Abacus. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".abacus/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Abacus-specific `devServer` flag.
...
Abacus should distinguish between:
...
Abacus should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Abacus should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Abacus
...
For runtime services specifically, Abacus should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Abacus-only dev-server flags
...
3. Inject resolved execution workspace details into `context.abacusWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Abacus instance can be reused through env/config injection when appropriate
...
- Abacus can decide whether to reuse or start a fresh service deterministically
...
3. Test against Abacus-on-Abacus workflows first.
...
7. Workspace runtime services are modeled generically, not as Abacus-specific dev-server toggles.
