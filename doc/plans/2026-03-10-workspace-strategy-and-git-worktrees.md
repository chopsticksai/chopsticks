...
`PAP-447` asks how RunEach should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same RunEach instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into RunEach:

- some operators use RunEach to manage RunEach and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as RunEach
- Claude and Codex expose different built-in affordances, so RunEach should not overfit to one tool

## Core Product Decision

RunEach should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by RunEach
...
- RunEach should not assume that every adapter can see or use a host filesystem path directly
...
If RunEach makes worktrees agent-first, it will blur:
...
- RunEach core resolves and records execution workspace state
...
That gives RunEach room to support:
...
- RunEach handles workspace runtime services under the hood using project/workspace policy
...
Once RunEach is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that RunEach cannot infer
...
- when issue moves to done, should RunEach auto-commit?
- should it auto-push?
...
RunEach already has the right foundation for a project-first model.
...
For server-spawned local adapters, RunEach already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that RunEach should mirror directly

Implication:

- for `codex_local`, RunEach should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but RunEach should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, RunEach should separate:
...
RunEach should be able to express intentions such as:
...
These are not specific to RunEach. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".runeach/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a RunEach-specific `devServer` flag.
...
RunEach should distinguish between:
...
RunEach should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, RunEach should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to RunEach
...
For runtime services specifically, RunEach should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as RunEach-only dev-server flags
...
3. Inject resolved execution workspace details into `context.runeachWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / RunEach instance can be reused through env/config injection when appropriate
...
- RunEach can decide whether to reuse or start a fresh service deterministically
...
3. Test against RunEach-on-RunEach workflows first.
...
7. Workspace runtime services are modeled generically, not as RunEach-specific dev-server toggles.
