<p align="center">
  <img src="doc/assets/header_zh.jpg" alt="Abacus — 用 AI 代理运转整家公司" width="720" />
</p>

<p align="center">
  <a href="#quickstart"><strong>快速开始</strong></a> &middot;
  <a href="__KEEP_ABACUS_COM_DOCS__"><strong>文档</strong></a> &middot;
...
> 基于 abacus fork，MIT 协议。

## Abacus 是什么？

# 面向"零人类公司"的开源编排系统

**如果 OpenClaw 是_员工_，Abacus 就是_公司_**

Abacus 提供一个 Node.js 后端 + React 控制面板，用于管理并运转一支 AI 代理团队。你接入自己的代理、分配目标，在一个控制台里追踪所有工作进度和成本。
...
> **即将推出：Clipmart** — 一键下载并运行整家公司。浏览预置的公司模板（完整组织架构、代理配置和技能），几秒内导入你的 Abacus 实例。
...
| ❌ 代理配置文件散落各处，重新发明任务管理、沟通协调的轮子。                                         | ✅ Abacus 开箱即提供组织架构、工单、委派和治理——管的是公司，不是一堆脚本。                        |
| ❌ 失控的循环在你毫不知情的情况下烧掉几百块的 token 并耗尽配额。                                   | ✅ 成本追踪实时显示 token 余额，超限自动节流，管理层用预算决定优先级。                               |
| ❌ 周期性任务（客服、社媒、报告）每次都要人工手动触发。                                             | ✅ 心跳按计划自动处理周期性工作，管理层负责监督。                                                     |
| ❌ 每次有想法，都要找到仓库、启动 Claude Code、开着标签页盯着它。                                  | ✅ 在 Abacus 里新建一个任务，代理会一直工作到完成，管理层负责审阅成果。                           |

<br/>

## 为什么 Abacus 不一样

Abacus 把那些棘手的编排细节处理对了。

|                          |                                                                                         |
| ------------------------ | --------------------------------------------------------------------------------------- |
| **原子性执行**           | 任务检出和预算执行是原子操作，不会重复干活，也不会超支。                               |
| **代理状态持久化**       | 代理在每次心跳之间延续同一个任务上下文，而不是从头开始。                               |
| **运行时技能注入**       | 代理可以在运行时学习 Abacus 的工作流和项目上下文，无需重新训练。                   |
...
| **不是工作流搭建器**   | 没有拖拽连线的流程图。Abacus 建模的是公司——有组织架构、目标、预算和治理。               |
| **不是 Prompt 管理器** | 代理自带提示词、模型和运行时。Abacus 管理的是它们所在的组织。                           |
| **不是单代理工具**     | 这是为团队设计的。如果你只有一个代理，可能用不上。如果你有二十个——一定用得上。            |
| **不是代码审查工具**   | Abacus 编排的是工作，不是 PR。代码审查流程请自行接入。                                   |
...
如果是独立开发者，可以用 Tailscale 在外出时访问本地的 Abacus，之后有需要再迁移到 Vercel 等云平台。

**能同时运行多家公司吗？**
可以。一个部署可以运行任意数量的公司，数据完全隔离。

**Abacus 和 OpenClaw、Claude Code 这类代理工具有什么区别？**
Abacus _使用_ 那些代理。它把它们编排成一家公司——有组织架构、预算、目标、治理和问责机制。

**为什么不直接把 OpenClaw 接到 Asana 或 Trello？**
代理编排有很多细节需要处理：谁锁定了哪个任务、如何维护会话状态、成本怎么监控、治理怎么落地——Abacus 把这些全部处理好了。

（支持接入自定义工单系统已在路线图中。）

**代理会持续运行吗？**
默认情况下，代理在计划心跳和事件触发（任务分配、@提及等）时运行。你也可以接入像 OpenClaw 这样的常驻代理。你负责提供代理，Abacus 负责协调。
