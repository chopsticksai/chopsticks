...
```sh
curl -sS -X POST "$RUNEACH_API_URL/api/companies/$RUNEACH_COMPANY_ID/skills/import" \
...
```sh
curl -sS -X POST "$RUNEACH_API_URL/api/companies/$RUNEACH_COMPANY_ID/skills/scan-projects" \
  -H "Authorization: Bearer $RUNEACH_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
```sh
curl -sS "$RUNEACH_API_URL/api/companies/$RUNEACH_COMPANY_ID/skills" \
  -H "Authorization: Bearer $RUNEACH_API_KEY"
```
...
```sh
curl -sS "$RUNEACH_API_URL/api/companies/$RUNEACH_COMPANY_ID/skills/<skill-id>" \
  -H "Authorization: Bearer $RUNEACH_API_KEY"

curl -sS "$RUNEACH_API_URL/api/companies/$RUNEACH_COMPANY_ID/skills/<skill-id>/files?path=SKILL.md" \
  -H "Authorization: Bearer $RUNEACH_API_KEY"
```
...
```sh
curl -sS -X POST "$RUNEACH_API_URL/api/agents/<agent-id>/skills/sync" \
  -H "Authorization: Bearer $RUNEACH_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "desiredSkills": [
...
```sh
curl -sS "$RUNEACH_API_URL/api/agents/<agent-id>/skills" \
  -H "Authorization: Bearer $RUNEACH_API_KEY"
```
...
```sh
curl -sS -X POST "$RUNEACH_API_URL/api/companies/$RUNEACH_COMPANY_ID/agents" \
  -H "Authorization: Bearer $RUNEACH_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "QA Browser Agent",
...
- Built-in RunEach runtime skills are still added automatically when required by the adapter.
