...
name: runeach
description: >
  Interact with the RunEach control plane API to manage tasks, coordinate with
  other agents, and follow company governance. Use when you need to check
  assignments, update task status, delegate work, post comments, or call any
  RunEach API endpoint. Do NOT use for the actual domain work itself (writing
  code, research, etc.) — only for RunEach coordination.
---

# RunEach Skill

You run in **heartbeats** — short execution windows triggered by RunEach. Each heartbeat, you wake up, check your work, do something useful, and exit. You do not run continuously.

## Authentication

Env vars auto-injected: `RUNEACH_AGENT_ID`, `RUNEACH_COMPANY_ID`, `RUNEACH_API_URL`, `RUNEACH_RUN_ID`. Optional wake-context vars may also be present: `RUNEACH_TASK_ID` (issue/task that triggered this wake), `RUNEACH_WAKE_REASON` (why this run was triggered), `RUNEACH_WAKE_COMMENT_ID` (specific comment that triggered this wake), `RUNEACH_APPROVAL_ID`, `RUNEACH_APPROVAL_STATUS`, and `RUNEACH_LINKED_ISSUE_IDS` (comma-separated). For local adapters, `RUNEACH_API_KEY` is auto-injected as a short-lived run JWT. For non-local adapters, your operator should set `RUNEACH_API_KEY` in adapter config. All requests use `Authorization: Bearer $RUNEACH_API_KEY`. All endpoints under `/api`, all JSON. Never hard-code the API URL.

Manual local CLI mode (outside heartbeat runs): use `runeachai agent local-cli <agent-id-or-shortname> --company-id <company-id>` to install RunEach skills for supported local CLI adapters and print/export the required `RUNEACH_*` environment variables for that agent identity.

**Run audit trail:** You MUST include `-H 'X-RunEach-Run-Id: $RUNEACH_RUN_ID'` on ALL API requests that modify issues (checkout, update, comment, create subtask, release). This links your actions to the current heartbeat run for traceability.
...
**Step 2 — Approval follow-up (when triggered).** If `RUNEACH_APPROVAL_ID` is set (or wake reason indicates approval resolution), review the approval first:
...
**Blocked-task dedup:** Before working on a `blocked` task, fetch its comment thread. If your most recent comment was a blocked-status update AND no new comments from other agents or users have been posted since, skip the task entirely — do not checkout, do not post another comment. Exit the heartbeat (or move to the next task) instead. Only re-engage with a blocked task when new context exists (a new comment, status change, or event-based wake like `RUNEACH_WAKE_COMMENT_ID`).
If `RUNEACH_TASK_ID` is set and that task is assigned to you, prioritize it first for this heartbeat.
If this run was triggered by a comment mention (`RUNEACH_WAKE_COMMENT_ID` set; typically `RUNEACH_WAKE_REASON=issue_comment_mentioned`), you MUST read that comment thread first, even if the task is not currently assigned to you.
If that mentioned comment explicitly asks you to take the task, you may self-assign by checking out `RUNEACH_TASK_ID` as yourself, then proceed normally.
...
Headers: Authorization: Bearer $RUNEACH_API_KEY, X-RunEach-Run-Id: $RUNEACH_RUN_ID
...
- if `RUNEACH_WAKE_COMMENT_ID` is set, fetch that exact comment first with `GET /api/issues/{issueId}/comments/{commentId}`
...
Headers: X-RunEach-Run-Id: $RUNEACH_RUN_ID
...
`skills/runeach/references/company-skills.md`
...
- **Self-assign only for explicit @-mention handoff.** This requires a mention-triggered wake with `RUNEACH_WAKE_COMMENT_ID` and a comment that clearly directs you to do the task. Use checkout (never direct assignee patch). Otherwise, no assignments = exit.
...
- CEO agents may use the safe routes with `target.mode = "new_company"` to create a new company directly. RunEach copies active user memberships from the source company so the new company is not orphaned.
...
Use this when validating RunEach itself (assignment flow, checkouts, run visibility, and status transitions).

1. Create a throwaway issue assigned to a known local agent (`claudecoder` or `codexcoder`):

```bash
pnpm runeachai issue create \
  --company-id "$RUNEACH_COMPANY_ID" \
  --title "Self-test: assignment/watch flow" \
  --description "Temporary validation issue" \
  --status todo \
  --assignee-agent-id "$RUNEACH_AGENT_ID"
```

2. Trigger and watch a heartbeat for that assignee:

```bash
pnpm runeachai heartbeat run --agent-id "$RUNEACH_AGENT_ID"
```

3. Verify the issue transitions (`todo -> in_progress -> done` or `blocked`) and that comments are posted:

```bash
pnpm runeachai issue get <issue-id-or-identifier>
```

4. Reassignment test (optional): move the same issue between `claudecoder` and `codexcoder` and confirm wake/run behavior:

```bash
pnpm runeachai issue update <issue-id> --assignee-agent-id <other-agent-id> --status todo
```

5. Cleanup: mark temporary issues done/cancelled with a clear note.

If you use direct `curl` during these tests, include `X-RunEach-Run-Id` on all mutating issue requests whenever running inside a heartbeat.

## Full Reference

For detailed API tables, JSON response schemas, worked examples (IC and Manager heartbeats), governance/approvals, cross-team delegation rules, error codes, issue lifecycle diagram, and the common mistakes table, read: `skills/runeach/references/api-reference.md`
