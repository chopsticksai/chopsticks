...
description: >
  Technical guide for creating a new Chopsticks agent adapter. Use when building
...
# Creating a Chopsticks Agent Adapter

An adapter bridges Chopsticks's orchestration layer to a specific AI agent runtime (Claude Code, Codex CLI, a custom process, an HTTP endpoint, etc.). Each adapter is a self-contained package that provides implementations for **three consumers**: the server, the UI, and the CLI.
...
packages/adapters/<name>/
  src/
    ...
    cli/
      ...
      format-event.ts   # Colored terminal output for `chopsticks run --watch`
  package.json
  tsconfig.json
```
...
## 2. Shared Types (`@chopsticks/adapter-utils`)

All adapter interfaces live in `packages/adapter-utils/src/types.ts`. Import from `@chopsticks/adapter-utils` (types) or `@chopsticks/adapter-utils/server-utils` (runtime helpers).
...
interface AdapterExecutionResult {
  ...
  clearSession?: boolean;         // true = tell Chopsticks to forget the stored session
}
...
```json
{
  ...
  "dependencies": {
    "@chopsticks/adapter-utils": "workspace:*",
    "picocolors": "^1.1.1"
  },
  "devDependencies": {
    "typescript": "^5.7.3"
...
The `agentConfigurationDoc` is read by LLM agents (including Chopsticks agents that create other agents). Write it as **routing logic**, not marketing copy. Include concrete "use when" and "don't use when" guidance so an LLM can decide whether this adapter is appropriate for a given task.
...
1. **Read config** — extract typed values from `ctx.config` using helpers (`asString`, `asNumber`, `asBoolean`, `asStringArray`, `parseObject` from `@chopsticks/adapter-utils/server-utils`)
2. **Build environment** — call `buildChopsticksEnv(agent)` then layer in `CHOPSTICKS_RUN_ID`, context vars (`CHOPSTICKS_TASK_ID`, `CHOPSTICKS_WAKE_REASON`, `CHOPSTICKS_WAKE_COMMENT_ID`, `CHOPSTICKS_APPROVAL_ID`, `CHOPSTICKS_APPROVAL_STATUS`, `CHOPSTICKS_LINKED_ISSUE_IDS`), user env overrides, and auth token
...
| `CHOPSTICKS_API_URL` | Server's own URL |
| `CHOPSTICKS_RUN_ID` | Current run id |
| `CHOPSTICKS_TASK_ID` | `context.taskId` or `context.issueId` |
| `CHOPSTICKS_WAKE_REASON` | `context.wakeReason` |
| `CHOPSTICKS_WAKE_COMMENT_ID` | `context.wakeCommentId` or `context.commentId` |
| `CHOPSTICKS_APPROVAL_ID` | `context.approvalId` |
| `CHOPSTICKS_APPROVAL_STATUS` | `context.approvalStatus` |
| `CHOPSTICKS_LINKED_ISSUE_IDS` | `context.issueIds` (comma-separated) |
| `CHOPSTICKS_API_KEY` | `authToken` (if no explicit key in config) |
...
Pretty-prints stdout lines for `chopsticks run --watch`. Use `picocolors` for coloring.
...
```ts
import { execute as myExecute, sessionCodec as mySessionCodec } from "@chopsticks/adapter-my-agent/server";
import { agentConfigurationDoc as myDoc, models as myModels } from "@chopsticks/adapter-my-agent";
...
const myAgentAdapter: ServerAdapterModule = {
  ...
  models: myModels,
  supportsLocalAgentJwt: true,  // true if agent can use Chopsticks API
...
import { parseMyAgentStdoutLine } from "@chopsticks/adapter-my-agent/ui";
import { MyAgentConfigFields } from "./config-fields";
import { buildMyAgentConfig } from "@chopsticks/adapter-my-agent/ui";
...
```ts
import { printMyAgentStreamEvent } from "@chopsticks/adapter-my-agent/cli";
...
- **Unknown session retry**: if resume fails with a "session not found" error, retry with a fresh session and return `clearSession: true` so Chopsticks wipes the stale session
...
Import from `@chopsticks/adapter-utils/server-utils`:
...
| `buildChopsticksEnv(agent)` | Standard `CHOPSTICKS_*` env vars |
...
- Package name: `@chopsticks/adapter-<kebab-name>`
...
- Default prompt: `"You are agent {{agent.id}} ({{agent.name}}). Continue your Chopsticks work."`
...
### Chopsticks Skills Injection

Chopsticks ships shared skills (in the repo's top-level `skills/` directory) that agents need at runtime — things like the `chopsticks` API skill and the `chopsticks-create-agent` workflow skill. Each adapter is responsible for making these skills discoverable by its agent runtime **without polluting the agent's working directory**.

**The constraint:** never copy or symlink skills into the agent's `cwd`. The cwd is the user's project checkout — writing `.claude/skills/` or any other files into it would contaminate the repo with Chopsticks internals, break git status, and potentially leak into commits.

**The pattern:** create a clean, isolated location for skills and tell the agent runtime to look there.

**How claude-local does it:**

1. At execution time, create a fresh tmpdir: `mkdtemp("chopsticks-skills-")`
...
async function buildSkillsDir(): Promise<string> {
  ...
  const entries = await fs.readdir(CHOPSTICKS_SKILLS_DIR, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.isDirectory()) {
      await fs.symlink(
        path.join(CHOPSTICKS_SKILLS_DIR, entry.name),
...
Codex has a global personal skills directory (`$CODEX_HOME/skills` or `~/.codex/skills`). The adapter symlinks Chopsticks skills there if they don't already exist. This is acceptable because it's the agent tool's own config directory, not the user's project.
...
**Skills as loaded procedures, not prompt bloat.** The Chopsticks skills (like `chopsticks` and `chopsticks-create-agent`) are designed as on-demand procedures: the agent sees skill metadata (name + description) in its context, but only loads the full SKILL.md content when it decides to invoke a skill. This keeps the base prompt small. When writing `agentConfigurationDoc` or prompt templates for your adapter, do not inline skill content — let the agent runtime's skill discovery do the work. The descriptions in each SKILL.md frontmatter act as routing logic: they tell the agent when to load the full skill, not what the skill contains.

**Explicit vs. fuzzy skill invocation.** For production workflows where reliability matters (e.g. an agent that must always call the Chopsticks API to report status), use explicit instructions in the prompt template: "Use the chopsticks skill to report your progress." Fuzzy routing (letting the model decide based on description matching) is fine for exploratory tasks but unreliable for mandatory procedures.

---

## 8. Security Considerations

Adapters sit at the boundary between Chopsticks's orchestration layer and arbitrary agent execution. This is a high-risk surface.
...
- `CHOPSTICKS_API_KEY` is injected by the server into the process environment, not the prompt
...
- Prefer minimal allowlists over open internet access. An agent that only needs to call the Chopsticks API and GitHub should not have access to arbitrary hosts.
