...
name: release
description: >
  Coordinate a full RunEach release across engineering verification, npm,
...
Run the full RunEach maintainer release workflow, not just an npm publish.
...
7. If running through RunEach, you have issue context for status updates and follow-up task creation.
...
RunEach now uses a commit-driven release model:
...
npm view @runeachai/runeach@canary version
```
...
- release summary in RunEach issue context
