...
└── .runeach.yaml            (RunEach vendor extension)
```
...
- Agent instructions go in the AGENTS.md body, not in .runeach.yaml
...
- For companies generated from a repo, add a references footer at the bottom of COMPANY.md body:
  `Generated from [repo-name](repo-url) with the company-creator skill from [RunEach](https://github.com/runeachai/runeach)`
...
- Citations and references: link to the source repo (if from-repo), link to the Agent Companies spec (https://agentcompanies.io/specification), and link to RunEach (https://github.com/runeachai/runeach)
- A "Getting Started" section explaining how to import: `runeachai company import --from <path>`
...
## .runeach.yaml Guidelines

The `.runeach.yaml` file is the RunEach vendor extension. It configures adapters and env inputs per agent.

### Adapter Rules

**Do not specify an adapter unless the repo or user context warrants it.** If you don't know what adapter the user wants, omit the adapter block entirely — RunEach will use its default. Specifying an unknown adapter type causes an import error.

RunEach's supported adapter types (these are the ONLY valid values):
...
In this example, only `release-engineer` appears because it needs `GH_TOKEN`. The other agents (ceo, cto, etc.) have no overrides, so they are omitted entirely from `.runeach.yaml`.
