...
- Swamifyx issue context now has incremental APIs: `GET /api/agents/me/inbox-lite`, `GET /api/issues/:id/heartbeat-context`, and comment delta queries via `GET /api/issues/:id/comments?after=...&order=asc`.
- The `chopsticks` skill now teaches agents to use those compact/incremental APIs first, while keeping full-thread fetches as a cold-start fallback.
...
- Runtime skill defaults now include `chopsticks`, `para-memory-files`, and `swamifyx-create-agent`. `create-agent-adapter` was moved to `.agents/skills/create-agent-adapter`.

Important follow-up finding from real-run review:

- `codex_local` currently injects Swamifyx skills into the shared Codex skills home (`$CODEX_HOME/skills` or `~/.codex/skills`) rather than mounting a worktree-local skill directory.
- If a Swamifyx-owned skill symlink already points at another live checkout, the adapter currently skips it instead of repointing it.
- In practice, this means a worktree can contain newer `skills/chopsticks/SKILL.md` guidance while Codex still follows an older checkout's skill content.
