...
- The `abacus` skill now teaches agents to use those compact/incremental APIs first, while keeping full-thread fetches as a cold-start fallback.
...
- Runtime skill defaults now include `abacus`, `para-memory-files`, and `swamifyx-create-agent`. `create-agent-adapter` was moved to `.agents/skills/create-agent-adapter`.
...
- In practice, this means a worktree can contain newer `skills/abacus/SKILL.md` guidance while Codex still follows an older checkout's skill content.
