...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@runeachai/db";
...
interface IncomingMessageWithContext extends IncomingMessage {
  runeachUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).runeachUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.runeachUpgradeContext = context;
