 ...
 *    (`~/.runeach/plugins/`) and `node_modules` for packages matching
 *    the `runeach-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published RunEach plugins.
 * Packages matching this pattern are considered RunEach plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "runeach-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: RunEachPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.runeach/plugins/ local directory
  | "npm"               // npm packages matching runeach-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: RunEachPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the runeach-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "runeach-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: RunEachPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover RunEach plugins installed as npm packages in the current
   * Node.js environment matching the "runeach-plugin-*" naming convention.
   *
   ...
   * the "runeachPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a RunEach plugin.
   * Throws if the package is a RunEach plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<RunEachPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: RunEachPluginManifestV1;
    newManifest: RunEachPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the RunEach plugin naming convention.
 * Accepts both the "runeach-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @runeachai/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "runeachPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const runeachPlugin = pkgJson["runeachPlugin"];
  if (
    runeachPlugin !== null &&
    typeof runeachPlugin === "object" &&
    !Array.isArray(runeachPlugin)
  ) {
    const manifestRelPath = (runeachPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: RunEachPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumRunEachVersion;
}
 ...
 *   packageName: "runeach-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: RunEachPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as RunEachPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a RunEach plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<RunEachPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a RunEach plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasRunEachPlugin = "runeachPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasRunEachPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<RunEachPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: RunEachPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @runeachai/shared exports). Run those workers through
