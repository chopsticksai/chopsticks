...
describe("agent local JWT", () => {
  const secretEnv = "RUNEACH_AGENT_JWT_SECRET";
  const ttlEnv = "RUNEACH_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "RUNEACH_AGENT_JWT_ISSUER";
  const audienceEnv = "RUNEACH_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "runeach";
    process.env[audienceEnv] = "runeach-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
