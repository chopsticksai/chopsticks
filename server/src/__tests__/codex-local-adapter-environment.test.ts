...
import { testEnvironment } from "@abacus/adapter-codex-local/server";
...
describe("codex_local environment diagnostics", () => {
  it("creates a missing working directory when cwd is absolute", async () => {
    const cwd = path.join(
      os.tmpdir(),
      `abacus-codex-local-cwd-${Date.now()}-${Math.random().toString(16).slice(2)}`,
      "workspace",
    );
  ...
  itWindows("runs the hello probe when Codex is available via a Windows .cmd wrapper", async () => {
    const root = path.join(
      os.tmpdir(),
      `abacus-codex-local-probe-${Date.now()}-${Math.random().toString(16).slice(2)}`,
    );
