...
describe("claude local skill sync", () => {
  ...
  const createAgentKey = "runeachai/runeach/runeach-create-agent";
  ...
  it("defaults to mounting all built-in RunEach skills when no explicit selection exists", async () => {
    ...
    expect(snapshot.entries.find((entry) => entry.key === runeachKey)?.required).toBe(true);
  ...
  it("normalizes legacy flat RunEach skill refs to canonical keys", async () => {
    const snapshot = await listClaudeSkills({
      agentId: "agent-3",
      companyId: "company-1",
      adapterType: "claude_local",
  ...
  it("shows host-level user-installed Claude skills as read-only external entries", async () => {
    const home = await makeTempDir("runeach-claude-user-skills-");
