...
<head>
    <!-- SWARMIFYX_FAVICON_START -->
    <link rel="icon" href="/favicon.ico" sizes="48x48" />
    <link rel="icon" href="/favicon.svg" type="image/svg+xml" />
    <link rel="icon" type="image/png" sizes="32x32" href="/favicon-32x32.png" />
    <link rel="icon" type="image/png" sizes="16x16" href="/favicon-16x16.png" />
    <!-- SWARMIFYX_FAVICON_END -->
</head>`;

describe("ui branding", () => {
  it("detects worktree mode from SWARMIFYX_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ SWARMIFYX_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ SWARMIFYX_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ SWARMIFYX_IN_WORKTREE: "false" })).toBe(false);
  });
  ...
  it("rewrites the favicon block for worktree instances only", () => {
    const branded = applyUiBranding(TEMPLATE, { SWARMIFYX_IN_WORKTREE: "true" });
