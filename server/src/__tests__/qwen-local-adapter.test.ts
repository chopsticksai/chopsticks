...
import { isQwenUnknownSessionError, parseQwenJsonl } from "@chopsticks/adapter-qwen-local/server";
import { parseQwenStdoutLine } from "@chopsticks/adapter-qwen-local/ui";
...
describe("qwen_local parser", () => {
  ...
  it("extracts structured questions", () => {
        ...
        message: {
          content: [
            { type: "output_text", text: "I have a question." },
            {
              ...
              prompt: "Which model?",
              choices: [
                { key: "plus", label: "Qwen Plus", description: "Better" },
                { key: "flash", label: "Qwen Flash" },
              ],
            },
          ],
        },
    ...
    expect(parsed.question).toEqual({
      ...
      choices: [
        { key: "plus", label: "Qwen Plus", description: "Better" },
        { key: "flash", label: "Qwen Flash", description: undefined },
      ],
    });
  });
});
...
describe("qwen_local stale session detection", () => {
  it("treats missing session messages as an unknown session error", () => {
    expect(isQwenUnknownSessionError("", "Session not found for id: abc")).toBe(true);
    expect(isQwenUnknownSessionError("", "Session not found: abc")).toBe(true);
  });
});
