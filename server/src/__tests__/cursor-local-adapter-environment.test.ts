...
import { testEnvironment } from "@swarmifyx/adapter-cursor-local/server";
...
const outPath = process.env.SWARMIFYX_TEST_ARGS_PATH;
if (outPath) {
...
describe("cursor environment diagnostics", () => {
  it("creates a missing working directory when cwd is absolute", async () => {
    const cwd = path.join(
      os.tmpdir(),
      `swarmifyx-cursor-local-cwd-${Date.now()}-${Math.random().toString(16).slice(2)}`,
      "workspace",
    );
  ...
  it("adds --yolo to hello probe args by default", async () => {
    const root = path.join(
      os.tmpdir(),
      `swarmifyx-cursor-local-probe-${Date.now()}-${Math.random().toString(16).slice(2)}`,
    );
    ...
    const result = await testEnvironment({
      ...
      config: {
        ...
        env: {
          ...
          SWARMIFYX_TEST_ARGS_PATH: argsCapturePath,
  ...
  it("does not auto-add --yolo when extraArgs already bypass trust", async () => {
    const root = path.join(
      os.tmpdir(),
      `swarmifyx-cursor-local-probe-extra-${Date.now()}-${Math.random().toString(16).slice(2)}`,
    );
    const binDir = path.join(root, "bin");
    const cwd = path.join(root, "workspace");
    const argsCapturePath = path.join(root, "args.json");
