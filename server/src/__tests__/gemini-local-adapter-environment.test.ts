...
import { testEnvironment } from "@swarmifyx/adapter-gemini-local/server";
...
describe("gemini_local environment diagnostics", () => {
  it("creates a missing working directory when cwd is absolute", async () => {
    const cwd = path.join(
      os.tmpdir(),
      `swarmifyx-gemini-local-cwd-${Date.now()}-${Math.random().toString(16).slice(2)}`,
      "workspace",
    );
  ...
  it("passes model and yolo flags to the hello probe", async () => {
    const root = path.join(
      os.tmpdir(),
      `swarmifyx-gemini-local-probe-${Date.now()}-${Math.random().toString(16).slice(2)}`,
    );
