...
function blockedHostnameMessage(hostname: string): string {
  return (
    `Hostname '${hostname}' is not allowed for this Abacus instance. ` +
    `If you want to allow this hostname, please run pnpm abacus allowed-hostname ${hostname}`
  );
}
...
export function privateHostnameGuard(opts: {
  ...
  return (req, res, next) => {
    ...
    if (!hostname) {
      const error = "Missing Host header. If you want to allow a hostname, run pnpm abacus allowed-hostname <host>.";
      if (wantsJson) {
