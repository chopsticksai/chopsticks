...
export function secretsCheck(config: RunEachConfig, configPath?: string): CheckResult {
  ...
  if (provider !== "local_encrypted") {
    return {
      ...
      canRepair: false,
      repairHint: `Run \`${publicCliCommand("configure --section secrets")}\` and set provider to local_encrypted`,
    };
  }

  const envMasterKey = process.env.RUNEACH_SECRETS_MASTER_KEY;
  if (envMasterKey && envMasterKey.trim().length > 0) {
    if (!decodeMasterKey(envMasterKey)) {
      return {
        ...
        status: "fail",
        message:
          "RUNEACH_SECRETS_MASTER_KEY is invalid (expected 32-byte base64, 64-char hex, or raw 32-char string)",
        canRepair: false,
        repairHint: "Set RUNEACH_SECRETS_MASTER_KEY to a valid key or unset it to use a key file",
      };
    }
    ...
    return withStrictModeNote(
      {
        name: "Secrets adapter",
        status: "pass",
        message: "Local encrypted provider configured via RUNEACH_SECRETS_MASTER_KEY",
      },
      config,
    );
  }

  const keyFileOverride = process.env.RUNEACH_SECRETS_MASTER_KEY_FILE;
  ...
  try {
    ...
    return {
      ...
      canRepair: false,
      repairHint: "Check file permissions or set RUNEACH_SECRETS_MASTER_KEY",
    };
  }
