...
import { createDb, instanceUserRoles, invites } from "@swarmifyx/db";
...
export async function bootstrapCeoInvite(opts: {
  ...
  try {
    ...
    p.log.info("If using embedded-postgres, start the SwarmifyX server and run this command again.");
  } finally {
