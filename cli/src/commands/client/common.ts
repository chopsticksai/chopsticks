...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    ...
    .option("-d, --data-dir <path>", `${PUBLIC_PRODUCT_NAME} data directory root (isolates state from ~/.chopsticks)`)
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.CHOPSTICKS_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const apiKey =
    options.apiKey?.trim() ||
    process.env.CHOPSTICKS_API_KEY?.trim() ||
    readKeyFromProfileEnv(profile);

  const companyId =
    options.companyId?.trim() ||
    process.env.CHOPSTICKS_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set CHOPSTICKS_COMPANY_ID, " +
      `or set context profile companyId via \`${publicCliCommand("context set")}\`.`,
    );
  }

  const api = new ChopsticksApiClient({ apiBase, apiKey });
  return {
    api,
    companyId,
    profileName,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.CHOPSTICKS_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.CHOPSTICKS_SERVER_PORT || "");
