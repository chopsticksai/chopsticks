    ...
    plugin
      ...
      .description(
        "Install a plugin from a local path or npm package.\n" +
          "  Examples:\n" +
          "    chopsticks plugin install ./my-plugin              # local path\n" +
          "    chopsticks plugin install @acme/plugin-linear      # npm package\n" +
          "    chopsticks plugin install @acme/plugin-linear@1.2  # pinned version",
      )
          ...
          for (const ex of rows) {
            console.log(
              `${pc.bold(ex.displayName)}  ${pc.dim(ex.pluginKey)}\n` +
                `  ${ex.description}\n` +
                `  ${pc.cyan(`chopsticks plugin install ${ex.localPath}`)}`,
            );
          }
        } catch (err) {
          handleCommandError(err);
