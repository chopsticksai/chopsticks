...
const DATA_DIR_OPTION_HELP =
  `${PUBLIC_PRODUCT_NAME} data directory root (isolates state from ~/.chopsticks)`;

program
  .name(PUBLIC_CLI_NAME)
  .description(`${PUBLIC_PRODUCT_NAME} CLI — setup, diagnose, and configure your instance`)
  .version("0.2.7");
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadChopsticksEnvFile(options.config);
});
...
program
  ...
  .option("--run", `Start ${PUBLIC_PRODUCT_NAME} immediately after saving config`, false)
  .action(onboard);

program
  .command("doctor")
  .description(`Run diagnostic checks on your ${PUBLIC_PRODUCT_NAME} setup`)
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "chopsticks")
...
program
  .command("run")
  .description(`Bootstrap local setup (onboard + doctor) and run ${PUBLIC_PRODUCT_NAME}`)
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", `Base URL for the ${PUBLIC_PRODUCT_NAME} server API`)
