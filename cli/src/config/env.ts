...
const JWT_SECRET_ENV_KEY = "CHOPSTICKS_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# Chopsticks environment variables",
    "# Generated by Chopsticks CLI commands",
...
export function resolveChopsticksEnvFile(configPath?: string): string {
...
export function loadChopsticksEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeChopsticksEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readChopsticksEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeChopsticksEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeChopsticksEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readChopsticksEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeChopsticksEnvEntries(next, filePath);
  return next;
}
