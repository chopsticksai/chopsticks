...
const JWT_SECRET_ENV_KEY = "SWARMIFYX_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# Swarmifyx environment variables",
    "# Generated by Swarmifyx CLI commands",
...
export function resolveSwarmifyxEnvFile(configPath?: string): string {
...
export function loadSwarmifyxEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeSwarmifyxEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readSwarmifyxEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeSwarmifyxEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeSwarmifyxEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readSwarmifyxEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeSwarmifyxEnvEntries(next, filePath);
  return next;
}
