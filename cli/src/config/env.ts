...
const JWT_SECRET_ENV_KEY = "ABACUS_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# Abacus environment variables",
    "# Generated by Abacus CLI commands",
...
export function resolveAbacusEnvFile(configPath?: string): string {
...
export function loadAbacusEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeAbacusEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readAbacusEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeAbacusEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeAbacusEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readAbacusEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeAbacusEnvEntries(next, filePath);
  return next;
}
