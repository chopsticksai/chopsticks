...
const JWT_SECRET_ENV_KEY = "RUNEACH_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# RunEach environment variables",
    "# Generated by RunEach CLI commands",
...
export function resolveRunEachEnvFile(configPath?: string): string {
...
export function loadRunEachEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeRunEachEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readRunEachEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeRunEachEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeRunEachEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readRunEachEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeRunEachEnvEntries(next, filePath);
  return next;
}
