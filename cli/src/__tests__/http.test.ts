...
describe("RunEachApiClient", () => {
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new RunEachApiClient({
    ...
    expect(headers["x-runeach-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new RunEachApiClient({ apiBase: "http://localhost:3100" });
