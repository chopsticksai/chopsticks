...
function createTempConfigPath() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "runeach-allowed-hostname-"));
  return path.join(dir, "config.json");
}

function writeBaseConfig(configPath: string) {
  const base: RunEachConfig = {
    $meta: {
      version: 1,
    ...
    database: {
      ...
      embeddedPostgresDataDir: "/tmp/runeach-db",
      embeddedPostgresPort: 54329,
      backup: {
        enabled: true,
        intervalMinutes: 60,
        retentionDays: 30,
        dir: "/tmp/runeach-backups",
      },
    },
    logging: {
      mode: "file",
      logDir: "/tmp/runeach-logs",
    },
    server: {
    ...
    storage: {
      provider: "local_disk",
      localDisk: { baseDir: "/tmp/runeach-storage" },
      s3: {
        bucket: "runeach",
        region: "us-east-1",
        prefix: "",
    ...
    secrets: {
      ...
      strictMode: false,
      localEncrypted: { keyFilePath: "/tmp/runeach-secrets/master.key" },
    },
  };
...
describe("allowed-hostname command", () => {
  it("adds and normalizes hostnames", async () => {
    ...
    const raw = JSON.parse(fs.readFileSync(configPath, "utf-8")) as RunEachConfig;
