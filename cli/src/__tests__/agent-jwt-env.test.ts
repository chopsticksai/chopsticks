...
function tempConfigPath(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "abacus-jwt-env-"));
...
describe("agent jwt env helpers", () => {
  ...
  it("writes .env next to explicit config path", () => {
    ...
    expect(contents).toContain("ABACUS_AGENT_JWT_SECRET=");
  });

  it("loads secret from .env next to explicit config path", () => {
    const configPath = tempConfigPath();
    const envPath = resolveAgentJwtEnvFile(configPath);
    fs.writeFileSync(envPath, "ABACUS_AGENT_JWT_SECRET=test-secret\n", { mode: 0o600 });

    const loaded = readAgentJwtSecretFromEnv(configPath);
    expect(loaded).toBe("test-secret");
    expect(process.env.ABACUS_AGENT_JWT_SECRET).toBe("test-secret");
  });

  it("doctor check passes when secret exists in adjacent .env", () => {
    const configPath = tempConfigPath();
    const envPath = resolveAgentJwtEnvFile(configPath);
    fs.writeFileSync(envPath, "ABACUS_AGENT_JWT_SECRET=check-secret\n", { mode: 0o600 });
  ...
  it("quotes hash-prefixed env values so dotenv round-trips them", () => {
    ...
    mergeAbacusEnvEntries(
      {
        ABACUS_WORKTREE_COLOR: "#439edb",
      },
      envPath,
    );

    const contents = fs.readFileSync(envPath, "utf-8");
    expect(contents).toContain('ABACUS_WORKTREE_COLOR="#439edb"');
    expect(readAbacusEnvEntries(envPath).ABACUS_WORKTREE_COLOR).toBe("#439edb");
  });
});
