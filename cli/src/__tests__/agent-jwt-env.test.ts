...
function tempConfigPath(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "runeach-jwt-env-"));
...
describe("agent jwt env helpers", () => {
  ...
  it("writes .env next to explicit config path", () => {
    ...
    expect(contents).toContain("RUNEACH_AGENT_JWT_SECRET=");
  });

  it("loads secret from .env next to explicit config path", () => {
    const configPath = tempConfigPath();
    const envPath = resolveAgentJwtEnvFile(configPath);
    fs.writeFileSync(envPath, "RUNEACH_AGENT_JWT_SECRET=test-secret\n", { mode: 0o600 });

    const loaded = readAgentJwtSecretFromEnv(configPath);
    expect(loaded).toBe("test-secret");
    expect(process.env.RUNEACH_AGENT_JWT_SECRET).toBe("test-secret");
  });

  it("doctor check passes when secret exists in adjacent .env", () => {
    const configPath = tempConfigPath();
    const envPath = resolveAgentJwtEnvFile(configPath);
    fs.writeFileSync(envPath, "RUNEACH_AGENT_JWT_SECRET=check-secret\n", { mode: 0o600 });
  ...
  it("quotes hash-prefixed env values so dotenv round-trips them", () => {
    ...
    mergeRunEachEnvEntries(
      {
        RUNEACH_WORKTREE_COLOR: "#439edb",
      },
      envPath,
    );

    const contents = fs.readFileSync(envPath, "utf-8");
    expect(contents).toContain('RUNEACH_WORKTREE_COLOR="#439edb"');
    expect(readRunEachEnvEntries(envPath).RUNEACH_WORKTREE_COLOR).toBe("#439edb");
  });
});
