 ...
 *
 * Bundles all workspace packages (@abacus/*) into a single file.
...
// to npm and resolved at runtime (e.g. @abacus/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@abacus/")) {
      externals.add(name);
    }
  }
