/**
 * esbuild configuration for building the chopsticks CLI for npm.
 *
 * Bundles all workspace packages (@chopsticks/*) into a single file.
...
// to npm and resolved at runtime (e.g. @chopsticks/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@chopsticks/")) {
      externals.add(name);
    }
  }
