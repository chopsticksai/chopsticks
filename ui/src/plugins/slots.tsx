 ...
 *
 * Plugin bundles are built with `external: ["@runeachai/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__runeachPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__runeachPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__runeachPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__runeachPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@runeachai/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@runeachai/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@runeachai/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@runeachai/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@runeachai/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__runeachPluginBridge__) {
 ...
 * self-register via `window.runeachPlugins.registerReactComponent()`.
 ...
 * of `@runeachai/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
