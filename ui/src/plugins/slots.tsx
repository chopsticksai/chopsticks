 ...
 *
 * Plugin bundles are built with `external: ["@abacus/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__abacusPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__abacusPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__abacusPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__abacusPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@abacus/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@abacus/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@abacus/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@abacus/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@abacus/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__abacusPluginBridge__) {
 ...
 * self-register via `window.abacusPlugins.registerReactComponent()`.
 ...
 * of `@abacus/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
