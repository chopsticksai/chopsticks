 ...
 *
 * Plugin bundles are built with `external: ["@chopsticks/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__chopsticksPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__chopsticksPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__chopsticksPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__chopsticksPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@chopsticks/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@chopsticks/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@chopsticks/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@chopsticks/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@chopsticks/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__chopsticksPluginBridge__) {
 ...
 * self-register via `window.chopsticksPlugins.registerReactComponent()`.
 ...
 * of `@chopsticks/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
