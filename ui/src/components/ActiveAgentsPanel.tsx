    ...
    <div>
      ...
      {runs.length === 0 ? (
        <div className="rounded-xl border border-border p-4">
          <p className="text-sm text-muted-foreground">{t("No recent agent runs.")}</p>
        </div>
      ) : (
    ...
    <div className={cn(
            ...
            <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
              <span>
                {isActive
                  ? t("Live now")
                  : run.finishedAt
                    ? t("Finished {time}", { time: relativeTime(run.finishedAt) })
                    : t("Started {time}", { time: relativeTime(run.createdAt) })}
              </span>
            </div>
          </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        <RunTranscriptView
          ...
          emptyMessage={hasOutput ? "Waiting for transcript parsing..." : isActive ? t("Waiting for output...") : "No transcript captured."}
        />
      </div>
    </div>
  );
