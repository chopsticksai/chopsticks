...
import type { LiveEvent } from "@chopsticks/shared";
...
function summarizeEntry(entry: TranscriptEntry): { text: string; tone: FeedTone } | null {
  ...
  if (entry.kind === "thinking") {
    const text = entry.text.trim();
    return text ? { text: translateText("[thinking] {text}", { text }), tone: "info" } : null;
  }
  if (entry.kind === "tool_call") {
    return { text: translateText("tool {name}", { name: entry.name }), tone: "tool" };
  }
  if (entry.kind === "tool_result") {
    const base = entry.content.trim();
    return {
      text: entry.isError
        ? translateText("tool error: {content}", { content: base })
        : translateText("tool result: {content}", { content: base }),
  ...
  const appendSummary = (entry: TranscriptEntry) => {
    ...
    if (entry.kind === "thinking" && entry.delta) {
      ...
      if (last && last.streamingKind === "thinking") {
        ...
        summarized.push({
          text: translateText("[thinking] {text}", { text }),
          tone: "info",
      ...
      socket.onmessage = (message) => {
        ...
        if (event.type === "heartbeat.run.status") {
          ...
          const item = createFeedItem(
            run,
            event.createdAt,
            translateText("run {status}", { status: getStatusLabel(status) }),
            tone,
            nextIdRef.current++,
          );
          if (item) appendItems([item]);
    ...
    <div className="rounded-lg border border-cyan-500/30 bg-background/80 overflow-hidden shadow-[0_0_12px_rgba(6,182,212,0.08)]">
      {runs.length > 0 ? (
        ...
        <div className="flex items-center px-3 py-2 border-b border-border/50">
          <span className="text-xs font-medium text-muted-foreground">{t("Recent run updates")}</span>
        </div>
      )}

      <div ref={bodyRef} className="max-h-[220px] overflow-y-auto p-2 font-mono text-[11px] space-y-1">
        {recent.length === 0 && (
          <div className="text-xs text-muted-foreground">{t("Waiting for run output...")}</div>
        )}
