...
import type { CostByBiller, CostByProviderModel } from "@abacus/shared";
    ...
    <Card>
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              ...
              {" · "}
              {row.providerCount === 1 ? t("1 provider") : t("{count} providers", { count: row.providerCount })}
              {" · "}
              {row.modelCount === 1 ? t("1 model") : t("{count} models", { count: row.modelCount })}
            </CardDescription>
          </div>
      ...
      <CardContent className="px-4 pb-4 pt-3 space-y-4">
        ...
        <div className="text-xs text-muted-foreground">
          {row.apiRunCount > 0 ? t("{count} metered runs", { count: row.apiRunCount }) : t("0 metered runs")}
          {" · "}
          {row.subscriptionRunCount > 0
            ? t("{count} subscription runs", { count: row.subscriptionRunCount })
            : t("0 subscription runs")}
          {" · "}
                  ...
                  <div key={entry.provider} className="flex items-center justify-between gap-2 text-xs">
                    ...
                    <div className="text-right tabular-nums">
                      ...
                      <div className="text-muted-foreground">
                        {t("{tokens} tok", { tokens: formatTokens(entry.inputTokens + entry.outputTokens) })}
                      </div>
                    </div>
                  </div>
                ))}
