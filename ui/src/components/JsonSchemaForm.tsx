...
export function labelFromKey(key: string, schema: JsonSchemaNode): string {
  if (schema.title) return translateText(schema.title);
...
export function validateField(
  ...
  if (isRequired && (value === undefined || value === null || value === "")) {
    return translateText("This field is required");
  }
  ...
  if (type === "string" || type === "secret-ref") {
    ...
    if (schema.minLength != null && str.length < schema.minLength) {
      return translateText("Must be at least {count} characters", { count: schema.minLength });
    }
    if (schema.maxLength != null && str.length > schema.maxLength) {
      return translateText("Must be at most {count} characters", { count: schema.maxLength });
    }
    if (schema.pattern) {
      ...
      if (schema.pattern.length <= MAX_PATTERN_LENGTH) {
        try {
          ...
          if (!re.test(str)) {
            return translateText("Must match pattern: {pattern}", { pattern: schema.pattern });
          }
        } catch {
  ...
  if (type === "number" || type === "integer") {
    ...
    if (isNaN(num)) return translateText("Must be a valid number");
    if (schema.minimum != null && num < schema.minimum) {
      return translateText("Must be at least {count}", { count: schema.minimum });
    }
    if (schema.maximum != null && num > schema.maximum) {
      return translateText("Must be at most {count}", { count: schema.maximum });
    }
    if (schema.exclusiveMinimum != null && num <= schema.exclusiveMinimum) {
      return translateText("Must be greater than {count}", { count: schema.exclusiveMinimum });
    }
    if (schema.exclusiveMaximum != null && num >= schema.exclusiveMaximum) {
      return translateText("Must be less than {count}", { count: schema.exclusiveMaximum });
    }
    if (type === "integer" && !Number.isInteger(num)) {
      return translateText("Must be a whole number");
    }
    if (schema.multipleOf != null && num % schema.multipleOf !== 0) {
      return translateText("Must be a multiple of {count}", { count: schema.multipleOf });
    }
  }
  ...
  if (type === "array") {
    ...
    if (schema.minItems != null && arr.length < schema.minItems) {
      return translateText("Must have at least {count} items", { count: schema.minItems });
    }
    if (schema.maxItems != null && arr.length > schema.maxItems) {
      return translateText("Must have at most {count} items", { count: schema.maxItems });
    }
  }
...
const SecretField = React.memo(({
  ...
  return (
    <FieldWrapper
      ...
      description={
        description ||
        translateText("This secret is stored securely via the RunEach secret provider.")
      }
      required={isRequired}
      error={error}
      disabled={disabled}
      ...
      <div className="relative">
        ...
        >
          ...
          <span className="sr-only">
            {isVisible ? translateText("Hide secret") : translateText("Show secret")}
          </span>
        </Button>
      </div>
    </FieldWrapper>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          {propSchema.description && (
            <p className="text-xs text-muted-foreground">
              {translateText(propSchema.description)}
            </p>
          )}
        </div>
        <Button
        ...
        >
          ...
          {isComplex ? translateText("Add item") : translateText("Add")}
        </Button>
      </div>
          ...
          >
            <div className="flex-1">
              <div className="mb-2 text-xs font-medium text-muted-foreground">
                {translateText("Item {index}", { index: index + 1 })}
              </div>
              <FormField
...
export function JsonSchemaForm({
  ...
  return (
    <div key={locale} className={cn("space-y-6", className)}>
