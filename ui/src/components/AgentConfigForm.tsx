...
export function AgentConfigForm(props: AgentConfigFormProps) {
  ...
  const hidePromptTemplate = props.hidePromptTemplate ?? false;
    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        ...
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          ...
          {isLocal && isCreate && !hidePromptTemplate && (
            <Field label="Prompt Template" hint={help.promptTemplate}>
              <MarkdownEditor
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("Permissions & Configuration")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("Permissions & Configuration")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            ...
            {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
              <>
                <Field label={t("Bootstrap prompt (legacy)")} hint={help.bootstrapPrompt}>
                  <MarkdownEditor
                    value={eff(
                      "adapterConfig",
                    ...
                    placeholder={t("Optional initial setup prompt for the first run")}
                    contentClassName="min-h-[44px] text-sm font-mono"
                    imageUploadHandler={async (file) => {
...
function AdapterTypeDropdown({
  ...
  function renderAdapterIcon(adapterType: string) {
    ...
    if (adapterType === "gemini_local") {
      return <GeminiLogoIcon className="h-3.5 w-3.5" />;
    }
