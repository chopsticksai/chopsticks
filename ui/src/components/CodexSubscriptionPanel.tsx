...
import {
  ...
  translateQuotaWindowValueLabel,
} from "@/lib/quotaWindowText";
...
function QuotaWindowRow({ window }: { window: QuotaWindow }) {
  const { t } = useI18n();
  const translatedLabel = translateQuotaWindowLabel(window.label, t);
  const translatedValueLabel = translateQuotaWindowValueLabel(window.valueLabel, t);
  const detail = (() => {
    const raw = detailText(window);
    if (!raw) return null;
    if (!window.resetsAt || raw !== window.resetsAt) return translateQuotaWindowDetail(raw, t);
    ...
    return (
      <div className="border border-border px-3.5 py-3">
        <div className="flex items-center justify-between gap-3">
          <div className="text-sm font-medium text-foreground">{translatedLabel}</div>
          {translatedValueLabel ? (
            <div className="text-sm font-semibold tabular-nums text-foreground">{translatedValueLabel}</div>
          ) : null}
        </div>
        {detail ? (
