...
import type { BudgetPolicySummary } from "@runeachai/shared";
...
function scopeTypeLabel(scopeType: BudgetPolicySummary["scopeType"], t: (key: string, values?: Record<string, string | number>) => string) {
  if (scopeType === "company") return t("Company");
  if (scopeType === "agent") return t("Agent");
  return t("Project");
}
    ...
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? t("{percent}% of limit", { percent: summary.utilizationPercent }) : t("No cap configured")}
        </div>
      </div>
      <div>
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          {t("Soft alert at {percent}%", { percent: summary.warnPercent })}
          {summary.paused && summary.pauseReason ? ` · ${summary.pauseReason} ${t("pause")}` : ""}
        </div>
      </div>
    </div>
  ) : (
      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-6">
          <div>
            ...
            <div className="mt-2 text-sm text-muted-foreground">{t(windowLabel(summary.windowKind))}</div>
          </div>
          <div
            className={cn(
          ...
          >
            ...
            {summary.paused ? t("Paused") : summary.status === "warning" ? t("Warning") : summary.status === "hard_stop" ? t("Hard stop") : t("Healthy")}
          </div>
        </div>
      ...
      <CardHeader className={cn("gap-3", compact ? "px-4 pt-4 pb-2" : "px-5 pt-5 pb-3")}>
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <CardDescription className="mt-1">{t(windowLabel(summary.windowKind))}</CardDescription>
          </div>
