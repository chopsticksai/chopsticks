...
type I18nTranslate = (text: string, values?: Record<string, string | number>) => string;
...
function generateReadmeFromSelection(
  ...
  if (agents.length > 0) {
    lines.push(`![${t("Org Chart")}](images/org-chart.png)`);
    lines.push("");
  }
  ...
  if (agents.length > 0) {
    ...
    for (const agent of agents) {
      const roleLabel = t(ROLE_LABELS[agent.role] ?? agent.role);
  ...
  const downloadMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      pushToast({
        ...
        body: t("{count} file(s) exported as {name}.zip", {
    ...
    <div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          >
            ...
            {downloadMutation.isPending
              ? t("Building export...")
              : t("Export {count} file(s)", { count: selectedCount })}
          </Button>
        </div>
      </div>
      ...
      <div className="grid h-[calc(100vh-12rem)] gap-0 xl:grid-cols-[19rem_minmax(0,1fr)]">
        ...
        <div className="min-w-0 overflow-y-auto pl-6">
          <ExportPreviewPane t={t} selectedFile={selectedFile} content={previewContent} allFiles={effectiveFiles} onSkillClick={handleSkillClick} />
        </div>
      </div>
    </div>
  );
