      ...
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          ...
          <span className="text-xs text-muted-foreground">
            {t("{count} item(s)", { count: conflicts.length })}
          </span>
        </div>
    ...
    <div className="mx-5 mt-3">
      <div className="rounded-md border border-border">
        <div className="flex items-center gap-2 border-b border-border px-4 py-2.5">
          ...
          <span className="text-xs text-muted-foreground">
            {t("{count} agent(s)", { count: agents.length })}
          </span>
        </div>
        <div className="divide-y divide-border">
          {agents.map((agent) => {
...
async function readLocalPackageZip(
  file: File,
  t: (text: string, values?: Record<string, string | number>) => string,
): Promise<{
  name: string;
  ...
  if (!/\.zip$/i.test(file.name)) {
    throw new Error(t("Select a .zip company package."));
  }
  const archive = await readZipArchive(await file.arrayBuffer());
  if (Object.keys(archive.files).length === 0) {
    throw new Error(t("No package files were found in the selected zip archive."));
  }
  return {
    name: file.name,
  ...
  const importMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        ...
        body: t("{company}: {count} agent(s) processed.", {
  ...
  return (
    <div>
      ...
      <div className="border-b border-border px-5 py-5 space-y-4">
        ...
        {sourceMode === "local" ? (
          <div className="rounded-md border border-dashed border-border px-3 py-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {localPackage && (
                <span className="text-xs text-muted-foreground">
                  {localPackage.name} {t("with")} {Object.keys(localPackage.files).length} {t("file")}
        ...
        <Field
          label={t("Target")}
          hint={t("Import into this company or create a new one.")}
        >
          <select
            className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
            value={targetMode}
          ...
          >
            ...
            <option value="existing">
              {t("Existing company")}: {selectedCompany?.name}
            </option>
          </select>
        </Field>
          ...
          <div className="sticky top-0 z-10 border-b border-border bg-background px-5 py-3">
            <div className="flex flex-wrap items-center gap-4 text-sm">
              ...
              <span className="text-muted-foreground">
                {selectedCount} / {totalFiles} {t("selected")}
              </span>
              {conflicts.length > 0 && (
                <span className="text-amber-500">
                  {t("{count} conflict(s)", { count: conflicts.length })}
                </span>
              )}
              {importPreview.errors.length > 0 && (
                <span className="text-destructive">
                  {t("{count} error(s)", { count: importPreview.errors.length })}
                </span>
              )}
            </div>
          </div>
          ...
          <div className="mx-5 mt-3 flex justify-end">
            ...
            >
              ...
              {importMutation.isPending
                ? t("Importing...")
                : t("Import {count} file(s)", { count: selectedCount })}
            </Button>
          </div>
            ...
            <aside className="flex flex-col border-r border-border overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto">
                <PackageFileTree
                  ...
                  renderFileExtra={(node, checked) => renderImportFileExtra(t, node, checked, renameMap)}
