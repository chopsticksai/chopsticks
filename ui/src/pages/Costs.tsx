  ...
  return (
    <Card>
      <CardHeader className="px-5 pt-5 pb-2">
        ...
        <CardDescription>{t("Account-level charges that do not map to a single inference request.")}</CardDescription>
      </CardHeader>
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          label={t("Debits")}
          value={formatCents(debitCents)}
          subtitle={t("{count} total events in range", { count: eventCount })}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("Credits")}
...
export function Costs() {
  ...
  const showOverviewLoading = (spendLoading || financeLoading || budgetLoading) && customReady;
  const overviewError = spendError ?? financeError ?? budgetError;
  const budgetScopeCopy: Record<"company" | "agent" | "project", { title: string; description: string }> = {
    company: {
      title: t("Company budgets"),
      description: t("Company-wide monthly policy."),
    },
    agent: {
      title: t("Agent budgets"),
      description: t("Recurring monthly spend policies for individual agents."),
    },
    project: {
      title: t("Project budgets"),
      description: t("Lifetime spend policies for execution-bound projects."),
    },
  };
      ...
      <div className="space-y-5">
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              ...
              subtitle={t("{tokens} tokens across request-scoped events", {
            ...
            <MetricTile
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ...
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("{spent} of {budget}", {
                        spent: formatCents(spendData.summary.spendCents),
            ...
            <MetricTile
              ...
              subtitle={t("{amount} estimated in range", {
              ...
              <div className="grid gap-4 xl:grid-cols-[1.3fr,1fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    ...
                    <CardDescription>{t("Request-scoped inference spend for the selected period.")}</CardDescription>
                  </CardHeader>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("Budget {amount}", { amount: formatCents(spendData.summary.budgetCents) })
                            : t("Unlimited budget")}
                        </div>
                      </div>
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                {row.apiRunCount > 0 || row.subscriptionRunCount > 0 ? (
                                  <div className="text-xs text-muted-foreground">
                                    {t("{api} api · {subscription} subscription", {
                                      api: row.apiRunCount,
                ...
                {(["company", "agent", "project"] as const).map((scopeType) => {
                  ...
                  return (
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold">{budgetScopeCopy[scopeType].title}</h2>
                        <p className="text-sm text-muted-foreground">{budgetScopeCopy[scopeType].description}</p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
                          <BudgetPolicyCard
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      ...
                      <CardDescription>
                        {t("Account-level financial events grouped by who charged or credited them.")}
                      </CardDescription>
                    </CardHeader>
