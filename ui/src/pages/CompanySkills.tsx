...
function sourceMeta(
  ...
  switch (sourceBadge) {
    case "skills_sh":
      return { icon: VercelMark, label: translatedSourceLabel ?? "skills.sh", managedLabel: t("skills.sh managed") };
    case "github":
      return isSkillsShManaged
        ? { icon: VercelMark, label: translatedSourceLabel ?? "skills.sh", managedLabel: t("skills.sh managed") }
        : { icon: Github, label: translatedSourceLabel ?? t("GitHub"), managedLabel: t("GitHub managed") };
    case "url":
...
function formatProjectScanSummary(
  ...
  return t("{summary} across {count} workspace(s).", {
    summary: parts.join(", "),
  ...
  return (
    <div>
      {filteredSkills.map((skill) => {
        ...
        const source = sourceMeta(t, skill.sourceBadge, skill.sourceLabel);
...
function SkillPane({
  ...
  const source = sourceMeta(t, detail.sourceBadge, detail.sourceLabel);
  const SourceIcon = source.icon;
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          {detail.editable ? (
            ...
            <div className="text-sm text-muted-foreground">
              {detail.editableReason ? t(detail.editableReason) : null}
            </div>
          )}
        </div>
            ...
            {detail.sourceType === "github" && (
              <div className="flex flex-wrap items-center gap-2">
                ...
                {updateStatus?.trackingRef && (
                  <span className="text-xs text-muted-foreground">{t("tracking {value}", { value: updateStatus.trackingRef })}</span>
                )}
                <Button
                  variant="ghost"
                  size="sm"
                ...
                {updateStatus?.supported && updateStatus.hasUpdate && (
                  ...
                  >
                    ...
                    {t("Install update{pin}", { pin: latestPin ? ` ${latestPin}` : "" })}
                  </Button>
                )}
...
export function CompanySkills() {
  ...
  const importSkill = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        ...
        body: t("{count} skill(s) added.", { count: result.imported.length }),
      });
  ...
  const scanProjects = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      await queryClient.invalidateQueries({ queryKey: queryKeys.companySkills.list(selectedCompanyId!) });
      const summary = formatProjectScanSummary(t, result);
