...
function formatLastRunTimestamp(t: (key: string, values?: Record<string, string | number>) => string, value: Date | string | null | undefined) {
...
function getConcurrencyPolicyLabel(t: (key: string, values?: Record<string, string | number>) => string, value: string) {
  switch (value) {
    case "coalesce_if_active":
      return t("If a run is already active, keep just one follow-up run queued.");
    case "always_enqueue":
      return t("Queue every trigger occurrence, even if the routine is already running.");
    case "skip_if_active":
      return t("Drop new trigger occurrences while a run is still active.");
    default:
...
function getCatchUpPolicyLabel(t: (key: string, values?: Record<string, string | number>) => string, value: string) {
  switch (value) {
    case "skip_missed":
      return t("Ignore windows that were missed while the scheduler or routine was paused.");
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        ...
        body: t("Add the first trigger to turn it into a live workflow."),
        tone: "success",
      });
      ...
      >
        <DialogContent showCloseButton={false} className="max-w-3xl gap-0 overflow-hidden p-0">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border/60 px-5 py-3">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground">{t("New routine")}</p>
              <p className="text-sm text-muted-foreground">
                {t("Define the recurring work first. Trigger setup comes next on the detail page.")}
              </p>
            </div>
            <Button
              variant="ghost"
          ...
          <div className="border-t border-border/60 px-5 py-3">
            <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
              <CollapsibleTrigger className="flex w-full items-center justify-between text-left">
                <div>
                  <p className="text-sm font-medium">{t("Advanced delivery settings")}</p>
                  <p className="text-sm text-muted-foreground">{t("Keep policy controls secondary to the work definition.")}</p>
                </div>
              ...
              <CollapsibleContent className="pt-3">
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">{getConcurrencyPolicyLabel(t, draft.concurrencyPolicy)}</p>
                  </div>
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">{getCatchUpPolicyLabel(t, draft.catchUpPolicy)}</p>
                  </div>
                </div>
              </CollapsibleContent>
            </Collapsible>
            ...
            <div className="flex flex-col gap-2 sm:items-end">
              ...
              {createRoutine.isError ? (
                <p className="text-sm text-destructive">
                  {createRoutine.error instanceof Error ? createRoutine.error.message : t("Failed to create routine")}
                </p>
              ) : null}
            </div>
          </div>
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="text-left text-xs text-muted-foreground border-b border-border">
                  ...
                  <th className="px-3 py-2 font-medium">{t("Last run")}</th>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5">
                        {routine.projectId ? (
                          <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            ...
                            />
                            <span className="truncate">{projectById.get(routine.projectId)?.name ?? t("Unknown")}</span>
                          </div>
                        ) : (
                      ...
                      <td className="px-3 py-2.5 text-muted-foreground">
                        <div>{formatLastRunTimestamp(t, routine.lastRun?.triggeredAt)}</div>
                        {routine.lastRun ? (
                          <div className="mt-1 text-xs">{getRoutineRunStatusLabel(t, routine.lastRun.status)}</div>
                        ) : null}
                      </td>
                      <td className="px-3 py-2.5" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center gap-3">
                          <button
                            ...
                            aria-label={enabled ? t("Disable {title}", { title: routine.title }) : t("Enable {title}", { title: routine.title })}
                      ...
                      <td className="px-3 py-2.5 text-right" onClick={(e) => e.stopPropagation()}>
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button variant="ghost" size="icon-sm" aria-label={t("More actions for {title}", { title: routine.title })}>
