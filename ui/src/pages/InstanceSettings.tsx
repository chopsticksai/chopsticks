...
import type { InstanceSchedulerHeartbeatAgent } from "@runeachai/shared";
...
export function InstanceSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("Failed to update heartbeat."));
    },
  });
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex gap-4 text-sm text-muted-foreground">
        ...
        <span>{t("{count} disabled", { count: disabledCount })}</span>
        <span>{t("{count} companies", { count: grouped.length })}</span>
      </div>
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {agent.schedulerActive ? t("On") : t("Off")}
                        </Badge>
                        <Link
                          ...
                          >
                            {saving
                              ...
                              : agent.heartbeatEnabled
                                ? t("Disable Timer Heartbeat")
                                : t("Enable Timer Heartbeat")}
                          </Button>
                        </span>
                      </div>
                    );
