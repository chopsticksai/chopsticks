...
function getRoutineTabLabel(t: (key: string, values?: Record<string, string | number>) => string, tab: RoutineTab) {
  switch (tab) {
    case "triggers":
      return t("Triggers");
    case "runs":
...
function getTriggerKindLabel(t: (key: string, values?: Record<string, string | number>) => string, kind: string) {
  switch (kind) {
    case "schedule":
      return t("Schedule");
    case "webhook":
...
function getTriggerKindOptionLabel(t: (key: string, values?: Record<string, string | number>) => string, kind: string) {
  const label = getTriggerKindLabel(t, kind);
  return kind === "webhook" ? t("{label} - COMING SOON", { label }) : label;
}

function getSigningModeLabel(t: (key: string, values?: Record<string, string | number>) => string, mode: string) {
  switch (mode) {
    case "bearer":
      return t("Bearer");
    case "hmac_sha256":
      return t("HMAC SHA-256");
    default:
      return mode;
  }
}

function getSigningModeDescription(t: (key: string, values?: Record<string, string | number>) => string, mode: string) {
  switch (mode) {
    case "bearer":
      return t("Expect a shared bearer token in the Authorization header.");
    case "hmac_sha256":
      return t("Expect an HMAC SHA-256 signature over the request using the shared secret.");
    default:
      return "";
  }
}

function getConcurrencyPolicyLabel(t: (key: string, values?: Record<string, string | number>) => string, value: string) {
  switch (value) {
    case "coalesce_if_active":
      return t("Keep one follow-up run queued while an active run is still working.");
    case "always_enqueue":
      return t("Queue every trigger occurrence, even if several runs stack up.");
    case "skip_if_active":
      return t("Drop overlapping trigger occurrences while the routine is already active.");
    default:
      return value.replaceAll("_", " ");
  }
}

function getCatchUpPolicyLabel(t: (key: string, values?: Record<string, string | number>) => string, value: string) {
  switch (value) {
    case "skip_missed":
      return t("Ignore schedule windows that were missed while the routine or scheduler was paused.");
    case "enqueue_missed_with_cap":
      return t("Catch up missed schedule windows in capped batches after recovery.");
    default:
      return value.replaceAll("_", " ");
  }
}

function getRoutineRunStatusLabel(t: (key: string, values?: Record<string, string | number>) => string, value: string) {
  return t(value.replaceAll("_", " "));
}

function getAutomationStatusLabel(t: (key: string, values?: Record<string, string | number>) => string, status: string) {
  switch (status) {
    case "archived":
      return t("Archived");
    case "active":
...
function getActivityActionLabel(t: (key: string, values?: Record<string, string | number>) => string, action: string) {
  switch (action) {
    case "issue.created":
      return t("created the issue");
    case "issue.updated":
      return t("updated the issue");
    case "issue.checked_out":
      return t("checked out the issue");
    case "issue.released":
      return t("released the issue");
    ...
    case "issue.document_created":
      return t("created a document");
    case "issue.document_updated":
      return t("updated a document");
    case "issue.document_deleted":
      return t("deleted a document");
    case "issue.deleted":
      return t("deleted the issue");
    case "agent.created":
      return t("created an agent");
    case "agent.updated":
      return t("updated the agent");
    case "agent.paused":
      return t("paused the agent");
    case "agent.resumed":
      return t("resumed the agent");
    case "agent.terminated":
      return t("terminated the agent");
    case "heartbeat.invoked":
      return t("invoked a heartbeat");
    case "heartbeat.cancelled":
      return t("cancelled a heartbeat");
...
function getActivityDetailKeyLabel(t: (key: string, values?: Record<string, string | number>) => string, key: string) {
  switch (key) {
    case "status":
      return t("status");
    case "priority":
    ...
    <div className="rounded-lg border border-border p-4 space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm font-medium">
          ...
          {trigger.label ?? getTriggerKindLabel(t, trigger.kind)}
        </div>
        <span className="text-xs text-muted-foreground">
          {trigger.kind === "schedule" && trigger.nextRunAt
            ? t("Next: {value}", { value: new Date(trigger.nextRunAt).toLocaleString() })
              ...
              >
                ...
                <SelectContent>
                  {signingModes.map((mode) => (
                    <SelectItem key={mode} value={mode}>{getSigningModeLabel(t, mode)}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
...
export function RoutineDetail() {
  ...
  const copySecretValue = async (label: string, value: string) => {
    try {
      ...
      pushToast({
        title: t("Failed to copy {label}", { label }),
  ...
  const automationLabel = getAutomationStatusLabel(t, routine.status === "archived" ? "archived" : automationEnabled ? "active" : "paused");
              ...
              >
                ...
                <SelectContent>
                  {concurrencyPolicies.map((value) => (
                    <SelectItem key={value} value={value}>{getConcurrencyPolicyLabel(t, value)}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <p className="text-xs text-muted-foreground">{getConcurrencyPolicyLabel(t, editDraft.concurrencyPolicy)}</p>
            </div>
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  {catchUpPolicies.map((value) => (
                    <SelectItem key={value} value={value}>{getCatchUpPolicyLabel(t, value)}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <p className="text-xs text-muted-foreground">{getCatchUpPolicyLabel(t, editDraft.catchUpPolicy)}</p>
            </div>
          </div>
        </CollapsibleContent>
      </Collapsible>
                  ...
                  <SelectContent>
                    {triggerKinds.map((kind) => (
                      <SelectItem key={kind} value={kind} disabled={kind === "webhook"}>
                        {getTriggerKindOptionLabel(t, kind)}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              {newTrigger.kind === "webhook" && (
                <>
                  <div className="space-y-1.5">
                    ...
                    <p className="text-xs text-muted-foreground">{getSigningModeDescription(t, newTrigger.signingMode)}</p>
                  </div>
                  <div className="space-y-1.5">
                    <Label className="text-xs">{t("Replay window (seconds)")}</Label>
                ...
                <div key={run.id} className="flex items-center justify-between px-3 py-2 text-sm">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <Badge variant={run.status === "failed" ? "destructive" : "secondary"} className="shrink-0">
                      {getRoutineRunStatusLabel(t, run.status)}
                    </Badge>
                    {run.trigger && (
            ...
            <div className="border border-border rounded-lg divide-y divide-border">
              {(activity ?? []).map((event) => (
                <div key={event.id} className="flex items-center justify-between px-3 py-2 text-xs gap-4">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="font-medium text-foreground/90 shrink-0">{getActivityActionLabel(t, event.action)}</span>
                    {event.details && Object.keys(event.details).length > 0 && (
                      <span className="text-muted-foreground truncate">
                        {Object.entries(event.details).slice(0, 3).map(([key, value], i) => (
                          <span key={key}>
                            {i > 0 && <span className="mx-1 text-border">·</span>}
                            <span className="text-muted-foreground/70">{getActivityDetailKeyLabel(t, key)}:</span>{" "}
