...
const ENABLED_INVITE_ADAPTERS = new Set(["claude_local", "codebuddy_local", "codex_local", "gemini_local", "qwen_local", "opencode_local", "cursor"]);
      ...
      <div className="mx-auto max-w-xl py-10">
        <div className="rounded-lg border border-border bg-card p-6">
          ...
          {(onboardingSkillUrl || onboardingSkillPath || onboardingInstallPath) && (
            <div className="mt-3 space-y-1 rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground">
              <p className="font-medium text-foreground">{t("Chopsticks skill bootstrap")}</p>
    ...
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">
          {invite.inviteType === "bootstrap_ceo" ? t("Bootstrap your Chopsticks instance") : t("Join this Chopsticks company")}
        </h1>
              ...
              >
                {joinAdapterOptions.map((type) => (
                  <option key={type} value={type} disabled={!ENABLED_INVITE_ADAPTERS.has(type)}>
                    {t(adapterLabels[type])}{!ENABLED_INVITE_ADAPTERS.has(type) ? t(" (Coming soon)") : ""}
                  </option>
                ))}
              </select>
            </label>
