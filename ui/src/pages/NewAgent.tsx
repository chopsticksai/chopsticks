...
import { AGENT_ROLES } from "@chopsticks/shared";
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">{t("Advanced agent configuration")}</p>
      </div>
              ...
              >
                {currentReportsTo ? (
                  <>
                    ...
                    {t("Reports to {name}", { name: currentReportsTo.name })}
                  </>
                ) : (
                  <>
                    <User className="h-3 w-3 text-muted-foreground" />
                    {isFirstAgent ? t("Reports to: N/A (CEO)") : t("Reports to...")}
                  </>
                )}
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-48 p-1" align="start">
              ...
              {(agents ?? []).map((a) => (
                ...
                >
                  ...
                  {a.name}
                  <span className="text-muted-foreground ml-auto">{t(roleLabels[a.role] ?? a.role)}</span>
                </button>
              ))}
            </PopoverContent>
          </Popover>
